...
function createAudioCommandHelpers({
  ...
  const skipMatchingOriginalAformat = envFlag("LOUDNORM_GPU_ORIGINAL_SKIP_MATCHING_AFORMAT");
  ...
  const cpuLoudnormProgressArgs = (background) => (background && envFlag("LOUDNORM_GPU_CPU_LOUDNORM_BACKGROUND_NO_PROGRESS") ? [] : ["-progress", "pipe:2"]);
