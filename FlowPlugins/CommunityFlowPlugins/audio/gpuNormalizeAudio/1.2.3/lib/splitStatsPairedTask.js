...
function createPairedSplitStatsTaskStarter({
  ...
  return (plan, partner, statsSampleRate, statsCache, statsErr, key, reason, parseLine, opts = {}) => {
    ...
    if (fusePairedCpuMeasure) {
      cpuLoudnormMeasurementStore.publishTask(plan, task.then((result) => result.cpuLoudnormRecord));
      cpuLoudnormMeasurementStore.publishTask(partner, partnerTask.then((result) => result.cpuLoudnormRecord));
    }
    return task;
  };
}
