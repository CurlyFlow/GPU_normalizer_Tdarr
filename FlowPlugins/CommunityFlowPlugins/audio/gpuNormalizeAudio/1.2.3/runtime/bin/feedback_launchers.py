...
def _safe_feedback_apply6_args(ctx, apply_chunk):
    reused_args = _reused_safe_feedback_apply6_args(ctx, apply_chunk)
...
def launch_sync_apply_kernel(ctx, result, apply_chunk, feedback_state_i_host, *, exact_chunk_unsafe, frame_offset, exact_prefinal_frames):
    ...
    if mode.segmented_safe_chunk:
        ...
        if can_parallel_skip_safe_feedback(ctx, apply_chunk, feedback_state_i_host):
            skip_apply_args = _safe_feedback_apply6_args(ctx, apply_chunk)
            apply_dt = launch_apply_kernel(ctx, 'safe-feedback-skip-parallel6', ctx.kernels.safe_feedback_skip_apply_fn, skip_apply_args, block_x=256)
            result.apply_kernel_time += apply_dt
            result.exact_safe_kernel_time += apply_dt
            result.exact_safe_apply_kernel_time += apply_dt
