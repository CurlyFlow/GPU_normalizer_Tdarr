...
def parse_runtime_args(argv=None):
    ...
    parser.add_argument('--chunk-mib', type=float, default=32.0)
...
def derive_runtime_config(args):
    ...
    exact_segmented_safe_feedback = exact_limiter_active and exact_prefill_output and env_flag('LOUDNORM_GPU_SEGMENTED_SAFE_FEEDBACK', True)
    ...
    exact_parallel_unsafe_feedback = exact_safe_feedback_slot_accum and env_flag('LOUDNORM_GPU_PARALLEL_UNSAFE_FEEDBACK', True)
    ...
    if exact_limiter_active:
        try:
            ...
            exact_risk_expand_windows = 3
    exact_risk_expand_before_windows = 4 if risk_expand_env is None else exact_risk_expand_windows
