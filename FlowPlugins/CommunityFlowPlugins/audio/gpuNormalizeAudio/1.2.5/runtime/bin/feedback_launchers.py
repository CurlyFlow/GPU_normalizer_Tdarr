...
def _source_safe_feedback_args(ctx, *, frame_specialized=False):
    frame_mode = 16 if frame_specialized else 0
    if not ctx.cfg.source_exact_precompute:
        return ctypes.c_uint64(0), ctypes.c_uint32(frame_mode)
    source_mode = 2
    if ctx.cfg.source_skip_limiter_lookahead_scan:
        source_mode += 1
    if ctx.cfg.source_fused_direct_feedback:
        source_mode += 2
    return ctx.bindings.source_exact_sums_arg, ctypes.c_uint32(source_mode + frame_mode)


def _reused_safe_feedback_apply6_args(ctx, apply_chunk, *, frame_specialized=False):
    if frame_specialized or not ctx.cfg.exact_apply_arg_reuse:
        return None
...
def _safe_feedback_apply6_args(ctx, apply_chunk, *, frame_specialized=False):
    reused_args = _reused_safe_feedback_apply6_args(ctx, apply_chunk, frame_specialized=frame_specialized)
    if reused_args is not None:
        return reused_args

    bindings = ctx.bindings
    frames_arg = ctypes.c_uint32(apply_chunk.this_frames)
    source_sums_arg, source_flag_arg = _source_safe_feedback_args(ctx, frame_specialized=frame_specialized)
    ...
    args._refs = (frames_arg, source_sums_arg, source_flag_arg)
    return args
...
def _launch_parallel_unsafe_feedback(ctx, result, apply_chunk, feedback_state_i_host, *, local_windows):
    ...
    if cfg.source_unsafe_skip_kernel and cfg.source_exact_precompute and ctx.args.channels == 2:
        ...
        unsafe_block_x = 256
    elif cfg.exact_unsafe_skip_kernel_6ch and ctx.args.channels == 6 and feedback_state_i_view(feedback_state_i_host).enabled('above_threshold'):
        unsafe_kernel = ctx.kernels.safe_feedback_skip_apply_fn
...
def launch_sync_apply_kernel(ctx, result, apply_chunk, feedback_state_i_host, *, exact_chunk_unsafe, frame_offset, exact_prefinal_frames):
    ...
    if mode.segmented_safe_chunk:
        ...
        if _launch_safe_run_scan_apply_6ch(ctx, result, apply_chunk, feedback_state_i_host, local_windows=local_windows):
            return
        if can_parallel_skip_safe_feedback(ctx, apply_chunk, feedback_state_i_host):
            state = feedback_state_i_view(feedback_state_i_host)
            frame_specialized = (
                getattr(cfg, 'exact_safe_skip_apply_frame_kernel', False)
                and state.enabled('above_threshold')
                and ctx.args.channels in (2, 6)
                and ctx.args.output_format == 'f64le'
            )
            skip_apply_args = _safe_feedback_apply6_args(ctx, apply_chunk, frame_specialized=frame_specialized)
