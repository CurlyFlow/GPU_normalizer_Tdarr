...
def initialize_runtime_buffers(cuda, args, buffers, *, cfg, state_bytes, b_coeffs, a_coeffs):
    ...
    if buffers.d_checkpoint_states.value:
        chk(cuda.cuMemsetD32_v2(buffers.d_checkpoint_states, 0, state_bytes // 4), 'cuMemsetD32(checkpoint_states)')
    if cfg.exact_limiter_active:
        ...
        if buffers.d_source_correction_flags.value:
            chk(cuda.cuMemsetD32_v2(buffers.d_source_correction_flags, 0, max(1, windows)), 'cuMemsetD32(source_correction_flags)')
...
class KernelArgBindings:
    ...
    checkpoint_states_arg: ctypes.c_uint64
    ...
    source_correction_flags_arg: ctypes.c_uint64
    ...
    def as_tuple(self):
        return (
            ...
            self.start_states_arg, self.checkpoint_states_arg, self.hist_energies_arg,
            self.hist_boundaries_arg, self.metrics_arg, self.source_short_ring_arg, self.source_exact_sums_arg,
            self.source_exact_audit_sums_arg, self.source_start_states_arg, self.source_energy_arg,
            self.source_correction_flags_arg, self.source_block_sums_arg, self.channels_arg, self.frame_window_arg, self.windows_arg,
...
def build_kernel_arg_bindings(args, buffers, cfg):
    return KernelArgBindings(
        ...
        checkpoint_states_arg=ctypes.c_uint64(buffers.d_checkpoint_states.value or 0),
        ...
        source_correction_flags_arg=ctypes.c_uint64(buffers.d_source_correction_flags.value or 0),
