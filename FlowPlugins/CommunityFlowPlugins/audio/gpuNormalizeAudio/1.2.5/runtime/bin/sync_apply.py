...
class ExactAsyncOutput:
    def __init__(self, cuda, buffers, *, output_chunk_bytes, direct_device_output=False, double_buffer=False):
        self.streams = []
        self.pinned_ptrs = []
        self.pinned_raws = []
        self.pinned_views = []
    ...
    def queue_output(self, cuda, result, buffers, *, this_output_bytes):
        ...
        host_slot = self.current_slot if self.double_buffer and self.direct_device_output else 0
...
def open_sync_apply_state(ctx, result, *, force_prefetch=False):
    ...
    async_output_requested = _env_flag('LOUDNORM_GPU_EXACT_APPLY_ASYNC_OUTPUT') or _env_flag('LOUDNORM_GPU_EXACT_APPLY_ASYNC_OUTPUT_DIRECT') or async_output_double_buffer
