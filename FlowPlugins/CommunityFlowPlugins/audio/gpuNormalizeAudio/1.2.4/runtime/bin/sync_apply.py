...
class ExactAsyncOutput:
    def __init__(self, cuda, buffers, *, output_chunk_bytes, direct_device_output=False):
        self.stream = ctypes.c_void_p()
        self.pinned_ptr = ctypes.c_void_p()
        self.pinned_raw = None
        ...
        chk(cuda.cuStreamCreate(ctypes.byref(self.stream), 0), 'cuStreamCreate(exact async output)')
        self.pinned_ptr, self.pinned_raw, self.pinned_view = alloc_pinned_host_buffer(cuda, output_chunk_bytes, 'exact async output')
    ...
    def select_output(self, ctx):
        if not self.direct_device_output:
            return
        slot = 0 if self.pending_slot != 0 else 1
    ...
    def finish_pending(self, cuda, result, fo):
        if self.pending_bytes <= 0:
            return
        t0 = time.perf_counter()
        chk(cuda.cuStreamSynchronize(self.stream), 'cuStreamSynchronize(exact async output)')
        wait_dt = time.perf_counter() - t0
        result.apply_d2h_time += wait_dt
        result.total_d2h_time += wait_dt
        out_view = self.pinned_view[:self.pending_bytes]
    ...
    def queue_output(self, cuda, result, buffers, *, this_output_bytes):
        ...
        if self.direct_device_output:
            ...
            self.pending_slot = self.current_slot
            self.current_slot = None
        else:
        ...
        chk(cuda.cuMemcpyDtoHAsync_v2(ctypes.c_void_p(self.pinned_ptr.value), output_ptr, this_output_bytes, self.stream), 'cuMemcpyDtoHAsync(exact output)')
        result.apply_d2h_time += time.perf_counter() - t0
        self.pending_bytes = this_output_bytes

    def cleanup(self, cuda):
        if self.stream.value:
            cuda.cuStreamDestroy_v2(self.stream)
            self.stream.value = None
        if self.pinned_ptr.value:
            cuda.cuMemFreeHost(self.pinned_ptr)
            self.pinned_ptr.value = None
...
def _copy_sync_output(ctx, result, fo, *, this_output_bytes, stage_t0, async_output=None):
...
def open_sync_apply_state(ctx, result, *, force_prefetch=False):
    ...
    async_output = None
    async_output_requested = _env_flag('LOUDNORM_GPU_EXACT_APPLY_ASYNC_OUTPUT') or _env_flag('LOUDNORM_GPU_EXACT_APPLY_ASYNC_OUTPUT_DIRECT')
    if async_output_requested and has_async_apply_api(cuda):
        try:
            async_output = ExactAsyncOutput(
                ...
                direct_device_output=_env_flag('LOUDNORM_GPU_EXACT_APPLY_ASYNC_OUTPUT_DIRECT'),
            )
        except Exception:
            async_output = None
...
def run_sync_apply_step(ctx, result, state):
    ...
    _copy_sync_output(ctx, result, state.fo, this_output_bytes=apply_chunk.this_output_bytes, stage_t0=stage_t0, async_output=state.async_output)
    ...
    if state.async_output is not None:
        try:
            if sys.exc_info()[0] is None:
                state.async_output.finish_pending(cuda, result, state.fo)
        finally:
