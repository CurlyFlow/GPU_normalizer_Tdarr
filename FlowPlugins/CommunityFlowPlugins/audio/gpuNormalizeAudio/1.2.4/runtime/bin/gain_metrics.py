...
def run_gain_metrics(cuda, args, *, cfg, buffers, kernels, bindings, emit_progress):
    ...
    fast_launch = getattr(cfg, 'gain_metrics_fast_launch', False)
    ...
    if has_metrics_kernel:
        ...
        if fast_launch:
            launch_kernel(cuda, 'source-port-gain-fast', gain_fn, gain_args)
            result.gain_kernel_time = 0.0
        else:
        ...
        if fast_launch:
            launch_kernel(cuda, 'source-port-metrics-fast', metrics_fn, metrics_args)
            result.metrics_kernel_time = 0.0
        else:
    ...
    if cfg.exact_use_risk_map:
        ...
        result.prelimiter_unsafe_flags, risk_stats = build_prelimiter_risk_map(
            ...
            gains_next_host,
            windows,
            cfg.ceiling * cfg.exact_risk_ceiling_scale,
        ...
        print(format_profile_stage('prelimiter_risk_map', [
            ...
            ('risk_ceiling_scale', f'{cfg.exact_risk_ceiling_scale:.6f}'),
            ('risk_ceiling', f'{(cfg.ceiling * cfg.exact_risk_ceiling_scale):.9f}'),
