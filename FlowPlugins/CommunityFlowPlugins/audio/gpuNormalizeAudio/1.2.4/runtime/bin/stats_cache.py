...
def _validate_header(header, args, cfg):
...
def _result_from_header(header):
...
def read_stats_cache_header(path, args, cfg):
    with open(path, 'rb') as fh:
        header = json.loads(fh.readline().decode('utf-8'))
    _validate_header(header, args, cfg)
    return _result_from_header(header)
...
def load_stats_cache(path, cuda, args, cfg, buffers):
    with open(path, 'rb') as fh:
        ...
        state_required_bytes = windows * args.channels * 4 * 8
        if state_bytes < state_required_bytes:
            raise RuntimeError(f'stats cache state below required windows: cache={state_bytes} required={state_required_bytes}')
    ...
    start_state_copy_bytes = min(start_states.itemsize * len(start_states), cfg.windows * args.channels * 4 * 8)
    chk(cuda.cuMemcpyHtoD_v2(buffers.d_start_states, ctypes.c_void_p(start_states.buffer_info()[0]), start_state_copy_bytes), 'cuMemcpyHtoD(stats cache start_states)')
