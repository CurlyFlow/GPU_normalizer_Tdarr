...
function niceCommand(command, niceLevel) {
...
function createSingleSplitStatsTaskStarter({
  ...
  return ({
    ...
    const statsNiceWhileCpu = Math.max(0, Math.min(19, intNum(process.env.LOUDNORM_GPU_ORIGINAL_SPLIT_STATS_NICE_WHILE_CPU, 0)));
    ...
    const cpuLoudnormPending = measureKey && cpuLoudnormMeasurementStore.hasTask(measureKey);
    const niceStatsForCpu = statsNiceWhileCpu > 0 && cpuLoudnormPending && !fuseCpuMeasure && !plan.stereoFallback;
    const runtimeCommand = niceCommand(wrapRuntimeProfile(statsPlan, plan, "stats"), niceStatsForCpu ? statsNiceWhileCpu : 0);
    ...
    if (envFlag("LOUDNORM_GPU_LOG_SPLIT_STATS_COMMAND")) {
      args.jobLog(`GPU normalize split stats diagnostics ${planLabel}: stats_cache=${statsCache} stats_err=${statsErr} raw_format=${plan.rawInputFormat}->${plan.rawGpuFormat} source=${plan.sourceInput} command=${runtimeCommand}`);
    }
    const taskPromise = runShell(runtimeCommand, {
      args,
      ...
      if (envFlag("LOUDNORM_GPU_LOG_SPLIT_STATS_COMMAND")) {
        const cacheValidation = validatePlanStatsCache(plan, statsSampleRate, statsCache);
        args.jobLog(`GPU normalize split stats diagnostics ${planLabel}: cache_ok=${cacheValidation.ok ? 1 : 0} cache_reason=${cacheValidation.reason || "ok"} cache_header=${cacheValidation.header ? JSON.stringify(cacheValidation.header) : ""}`);
      }
