...
function createPairedApplyConfig({ args, useGpuSourcePort, useStreamingSourcePort }) {
  const pairStereoFallbackApply = envFlag("LOUDNORM_GPU_PAIR_FALLBACK_APPLY", false) && useGpuSourcePort && useStreamingSourcePort && args.platform !== "win32";
  ...
  const pairStereoFallbackApplyOriginalFirstDecode = pairStereoFallbackApply && envFlag("LOUDNORM_GPU_PAIR_FALLBACK_APPLY_ORIGINAL_FIRST_DECODE", true);
  const pairStereoFallbackApplyDirectMuxOriginalOnly = pairStereoFallbackApply && envFlag("LOUDNORM_GPU_PAIR_FALLBACK_APPLY_DIRECT_MUX_ORIGINAL_ONLY");
  const pairStereoFallbackApplyEncodedFifoMuxOriginal = pairStereoFallbackApply && String(process.env.LOUDNORM_GPU_PAIR_FALLBACK_APPLY_ENCODED_FIFO_MUX || "").toLowerCase() === "original";
  const pairStereoFallbackApplyDirectMux = pairStereoFallbackApply && (envFlag("LOUDNORM_GPU_PAIR_FALLBACK_APPLY_DIRECT_MUX", false) || pairStereoFallbackApplyDirectMuxOriginalOnly || pairStereoFallbackApplyEncodedFifoMuxOriginal);
  ...
  const pairStereoFallbackApplyDirectFifoWrite = pairStereoFallbackApply && envFlag("LOUDNORM_GPU_PAIR_FALLBACK_APPLY_DIRECT_FIFO_WRITE", true);
  ...
  return {
    ...
    pairStereoFallbackApplyDirectMuxOriginalOnly,
    ...
    pairStereoFallbackApplyEncodedFifoMuxOriginal,
