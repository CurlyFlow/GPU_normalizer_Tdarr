...
function directMuxAacOptionsForSpec(spec, options) {
  if (spec.kind !== "directNormalized" || !spec.plan) return options;
  return {
    aacCoderFast: options.aacCoderFast,
    disableAacTools: options.disableAacTools
      || (!spec.plan.stereoFallback && options.originalDisableAacTools)
      || (spec.plan.stereoFallback && options.stereoFallbackDisableAacTools),
  };
}

function directMuxInputFormatForPlan(plan) {
...
function encodedDirectNormalizedSpec(plan, inputIndex) {
  return {
    kind: "encodedDirectNormalized",
    mapArg: `${inputIndex}:a:0`,
    language: plan.language,
    plan,
  };
...
function pushOutputAudioCodecs(args, audioSpecs, {
  ...
  audioSpecs.forEach((spec, audioIdx) => {
    ...
    if (spec.kind === "encodedDirectNormalized") {
      args.push(`-c:a:${audioIdx}`, "copy");
      return;
    }
    if (spec.kind === "directNormalized") {
      ...
      pushAacEncodeArgs(args, audioIdx, audioBitrate, directMuxAacOptionsForSpec(spec, directMuxAacOptions));
...
function buildCopyOriginalCommand({ ffmpegPath, inputFile, container, audioStreams, audioPlansToCopy = null, audioBitrate = "192k", tmpOutputFilePath }) {
  ...
  copyArgs.push(...mp4TrueHdOutputArgs(container, copiedTrueHdStreams));
  copyArgs.push(tmpOutputFilePath);
...
function buildFinalMuxCommand({ ffmpegPath, inputFile, audioPlans, skippedAudioPlans, container, audioStreams, audioBitrate = "192k", tmpOutputFilePath }) {
  ...
  muxArgs.push(...mp4TrueHdOutputArgs(container, copiedTrueHdStreams));
  muxArgs.push(tmpOutputFilePath);
...
function buildDirectMuxCommand({
  ...
  tmpOutputFilePath,
  rawInputAudioArgs,
  rawInputPathFor = (plan) => plan.fifoOutput,
  encodedInputPlanIds = [],
  encodeSampleRateArgsFor,
  encodeThreadArgs,
}) {
  if (!enabled) return "";
  const encodedInputPlanIdSet = new Set(encodedInputPlanIds);

  const directMuxAacOptions = {
    aacCoderFast: envFlag("LOUDNORM_GPU_AAC_CODER_FAST", true),
    disableAacTools: envFlag("LOUDNORM_GPU_AAC_DISABLE_TOOLS"),
    originalDisableAacTools: envFlag("LOUDNORM_GPU_ORIGINAL_AAC_DISABLE_TOOLS"),
    stereoFallbackDisableAacTools: envFlag("LOUDNORM_GPU_STEREO_FALLBACK_AAC_DISABLE_TOOLS"),
  };
  const directMuxArgs = [
  ...
  for (const plan of directMuxInputPlans) {
    ...
    if (encodedInputPlanIdSet.has(plan.idx)) {
      directMuxArgs.push("-f", "aac", "-i", rawInputPathFor(plan));
    } else {
      directMuxArgs.push("-f", directMuxInputFormatForPlan(plan), ...rawInputAudioArgs(plan, values), "-i", rawInputPathFor(plan));
    }
  }
  const audioSpecs = [
    ...audioPlans.map((plan) => {
      ...
      if (encodedInputPlanIdSet.has(plan.idx)) return encodedDirectNormalizedSpec(plan, inputIndex);
...
function buildDirectRawAudioEncodeCommand({
  enabled,
  ffmpegPath,
  noProgress,
  quietLog,
  ...
  rawInputPath,
  outputFormat = "",
  outputPath = plan.normalizedAudio,
  encodeSampleRateArgsFor,
  encodeThreadArgs,
  audioBitrate = "192k",
}) {
  ...
  const encodeArgs = [
    ffmpegPath, "-hide_banner", ...(noProgress || quietLog ? ["-loglevel", "error"] : []), "-nostats", "-nostdin",
    ...(noProgress ? [] : ["-progress", "pipe:2"]), "-y",
    "-f", directMuxInputFormatForPlan(plan), ...rawInputAudioArgs(plan, values), "-i", rawInputPath,
  ];
  if (encodeRateArgs[0] === "-ar" && encodeRateArgs[1]) encodeArgs.push("-ar", encodeRateArgs[1]);
  if (codecThreadArgs[0] === "-threads" && codecThreadArgs[1]) encodeArgs.push("-threads", codecThreadArgs[1]);
  pushAacEncodeArgs(encodeArgs, 0, audioBitrate, directMuxAacOptionsForSpec(directNormalizedSpec(plan, 0, values), directMuxAacOptions));
  if (outputFormat) encodeArgs.push("-f", outputFormat);
  encodeArgs.push(outputPath);
  return renderShellCommand(encodeArgs);
}
