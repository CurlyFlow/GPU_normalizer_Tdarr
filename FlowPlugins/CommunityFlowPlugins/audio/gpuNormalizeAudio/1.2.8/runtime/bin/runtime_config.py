...
RUNTIME_CONFIG_FIELDS = (
    ...
    'exact_skip_apply_rolling_ring_6ch',
    'exact_safe_skip_6ch_single_wrap',
    'exact_safe_run_scan_apply_6ch',
    'exact_safe_run_scan_apply_6ch_boundary_halo_windows',
...
def _runtime_config_values(*, input_sample_bytes, streaming_io, stats_cache_only, sections):
    ...
    values['encode_pipe_f32'] = bool(env_flag('LOUDNORM_GPU_ENCODE_PIPE_F32') and values.get('output_sample_bytes') == 8 and not stats_cache_only)
    return values
...
def parse_runtime_args(argv=None):
    ...
    parser.add_argument('--ingress-rate', type=int, default=0)
    ...
    parser.add_argument('--emit-first-pass-input-json', action='store_true', help='Emit first-pass input JSON after the stats stage and skip apply/output metrics.')
...
def derive_runtime_config(args):
    ...
    if args.ingress_rate and args.ingress_rate != args.rate and not args.streaming_io:
        raise SystemExit('--ingress-rate requires --streaming-io')
    if args.emit_first_pass_input_json and not args.stats_cache_only:
        raise SystemExit('--emit-first-pass-input-json requires --stats-cache-only')
...
def apply_stats_result_to_config(cfg, stats_result):
    ...
    updated = _runtime_config_from_sections(
    ...
    if hasattr(cfg, '_ffmpeg_input_metric_values'):
        updated._ffmpeg_input_metric_values = cfg._ffmpeg_input_metric_values
    return updated
