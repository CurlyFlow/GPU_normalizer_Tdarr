...
def prepare_sync_apply_chunk(ctx, result, fi, exact_stream_window, *, done_bytes, frame_offset, exact_use_final_flush, exact_prefinal_frames, exact_final_flush_frames, exact_safe_skip_chunk_bytes=None):
    args = ctx.args
    cfg = ctx.cfg
    ...
    if cfg.exact_limiter_active:
        exact_plan = plan_exact_limiter_chunk(
            ...
            prelimiter_run_end_window=ctx.prelimiter_run_end_window,
            exact_prefill_output=cfg.exact_prefill_output,
            exact_safe_chunk_bytes=exact_safe_skip_chunk_bytes or cfg.exact_safe_chunk_bytes,
            exact_unsafe_chunk_bytes=cfg.exact_unsafe_chunk_bytes,
        )
        ...
        copy_input_bytes = None
        if cfg.streaming_io:
            if getattr(cfg, 'exact_stream_stage_device', False):
                copy_t0 = time.perf_counter()
                input_bytes = exact_stream_window.stage_device(ctx.cuda, ctx.buffers.d_in, input_base_frame, input_frames)
        ...
        output_sums_arg = bindings.channel_sums_arg if _inline_first_pass_output_sums(ctx) else ctypes.c_uint64(0)
        ...
        return SyncApplyChunk(
            ...
            copy_input_bytes=copy_input_bytes,
            ...
            arg_refs=(input_base_arg, input_frames_arg, output_frames_arg, source_faithful_stereo_arg, output_sums_arg),
        )
    ...
    host_view = memoryview(host_in)
    if cfg.streaming_io:
        got = 0
        while got < this_bytes:
            chunk_got = fi.readinto(host_view[got:this_bytes])
            if not chunk_got:
                break
            got += chunk_got
        if got < this_bytes:
            if got % cfg.frame_bytes:
                raise RuntimeError('streaming apply ended with a partial frame')
            host_view[got:this_bytes] = b'\x00' * (this_bytes - got)
    else:
        got = fi.readinto(host_view[:this_bytes])
        if got != this_bytes:
