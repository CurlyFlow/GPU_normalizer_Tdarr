...
KERNEL_HANDLE_ATTRS = (
    ...
    'exact_sums_fn',
    'channel_sums_fn',
    'channel_sums_unroll4_fn',
    'channel_checkpoint_states_fn',
    'channel_sums_warp_fn',
    'channel_sums_no_peaks_fn',
    'channel_sums_source_energy_fn',
    'paired_channel_sums_fn',
    'peak_fn',
    ...
    'source_selective_short_raw_sums_fn',
    'source_mark_short_raw_corrections_fn',
    'source_apply_short_raw_corrections_fn',
    ...
    'metrics_fn',
    'apply_fn',
    'convert_f64_to_f32_fn',
    'prefill_apply_fn',
    'safe_feedback_energy_fn',
    'safe_feedback_window_fn',
    'safe_feedback_output_state_fn',
...
CUDA_KERNEL_SYMBOLS = (
    ...
    'kweight_window_sums_f64_paired_combined_kernel',
    'kweight_window_checkpoint_states_f64_channel_kernel',
    'kweight_window_sums_f64_channel_kernel',
    'kweight_window_sums_f64_channel_unroll4_kernel',
    'kweight_window_sums_f64_channel_warp_kernel',
    'kweight_window_sums_f64_channel_no_peaks_kernel',
    ...
    'safe_feedback_output_state_f64_kernel',
    ...
    'source_stereo_apply_short_raw_corrections_f64_kernel',
    'source_stereo_mark_short_raw_corrections_f64_kernel',
    'source_stereo_selective_hist4_sums_f64_kernel',
    'source_stereo_selective_short_raw_sums_f64_kernel',
...
def resolve_kernel_load_plan(args, cfg):
    ...
    if cfg.exact_limiter_active and input_format == 'f64le' and not cfg.exact_stats_stream and not cache_input_setup_slim:
        specs.extend([
            ...
            *([_spec('channel_sums_unroll4_fn', 'kweight_window_sums_f64_channel_unroll4_kernel', 'channel-sums-unroll4')] if getattr(cfg, 'exact_channel_stats_unroll4', False) else []),
            *([_spec('channel_checkpoint_states_fn', 'kweight_window_checkpoint_states_f64_channel_kernel', 'channel-checkpoint-states')] if getattr(cfg, 'exact_channel_stats_checkpoint_replay', False) else []),
            *([_spec('channel_sums_warp_fn', 'kweight_window_sums_f64_channel_warp_kernel', 'channel-sums-warp')] if getattr(cfg, 'exact_channel_stats_warp', False) else []),
            *([_spec('channel_sums_no_peaks_fn', 'kweight_window_sums_f64_channel_no_peaks_kernel', 'channel-sums-no-peaks')] if getattr(cfg, 'exact_channel_stats_no_peaks', False) else []),
            ...
            *([_spec('paired_channel_sums_fn', 'kweight_window_sums_f64_paired_combined_kernel', 'paired-channel-sums')] if getattr(cfg, 'exact_paired_stats_fused_kernel', False) else []),
        ...
        if cfg.source_exact_precompute and not getattr(cfg, 'cache_input_lean_source', False):
            ...
            if cfg.source_precompute_from_channel_sums:
                ...
                if cfg.source_channel_hist4_exact or cfg.source_channel_hist4_boundary or cfg.source_channel_short_exact or cfg.source_channel_short_boundary or getattr(cfg, 'source_channel_short_raw_boundary', False):
                    ...
                    if getattr(cfg, 'source_channel_short_raw_boundary', False):
                        specs.extend([
                            _spec('source_mark_short_raw_corrections_fn', 'source_stereo_mark_short_raw_corrections_f64_kernel', 'source-stereo-mark-short-raw-corrections'),
                            _spec('source_apply_short_raw_corrections_fn', 'source_stereo_apply_short_raw_corrections_f64_kernel', 'source-stereo-apply-short-raw-corrections'),
                        ])
        ...
        if cfg.exact_prefix_channel_stats or cfg.exact_safe_feedback_prefix or getattr(cfg, 'exact_channel_stats_checkpoint_replay', False):
            specs.extend([
    ...
    if not getattr(cfg, 'stats_cache_only_slim', False):
        ...
        if getattr(cfg, 'encode_pipe_f32', False):
            specs.append(_spec('convert_f64_to_f32_fn', 'convert_f64_to_f32_kernel', 'convert-f64-to-f32'))
        ...
        if cfg.exact_segmented_safe_feedback:
            specs.extend([
                ...
                *([_spec('safe_feedback_output_state_fn', 'safe_feedback_output_state_f64_kernel')] if (getattr(cfg, 'exact_safe_feedback_prefix_post_fill', False) or getattr(cfg, 'exact_safe_run_scan_apply_post_fill', False)) else []),
