...
def _debug_decode_reads():
    return env_flag('LOUDNORM_GPU_DEBUG_DECODE_READS')


def _debug_decode_log(message):
    if _debug_decode_reads():
...
class FrameChunkReader:
    ...
    def __enter__(self):
        ...
        _debug_decode_log(
            f'decode_reader_start label={self.label!r} streaming={int(bool(self.streaming_io))} '
            f'pid={getattr(self.proc, "pid", None)} nbytes={self.nbytes} chunk_bytes={self.chunk_bytes} '
            f'frame_bytes={self.frame_bytes} direct_payload={int(self.direct_payload)} '
            f'direct_bytearray={int(self.direct_bytearray_payload)}'
        )
        ...
        if self.prefetch_enabled:
            ...
            self.prefetch_aligned_fastpath = env_flag('LOUDNORM_GPU_PREFETCH_ALIGNED_FASTPATH', True)
            self.prefetch_thread.start()
        return self

    def __exit__(self, exc_type, exc, tb):
        _debug_decode_log(
            f'decode_reader_close label={self.label!r} done_bytes={self.done_bytes} '
            f'frame_offset={self.frame_offset} exc={getattr(exc_type, "__name__", None)} '
            f'pid={getattr(self.proc, "pid", None)} poll={self.proc.poll() if self.proc is not None else None}'
        )
    ...
    def __iter__(self):
        ...
        while True:
            if self.streaming_io:
                ...
                if not data:
                    _debug_decode_log(
                        f'decode_reader_eof label={self.label!r} done_bytes={self.done_bytes} '
                        f'frame_offset={self.frame_offset} pid={getattr(self.proc, "pid", None)} '
                        f'poll={self.proc.poll() if self.proc is not None else None}'
                    )
                    if stream_tail:
    ...
    def _prefetch_worker(self):
        ...
        try:
            while self.prefetch_stop is not None and not self.prefetch_stop.is_set():
                ...
                if not data:
                    _debug_decode_log(
                        f'decode_reader_eof label={self.label!r} done_bytes={done_bytes} '
                        f'frame_offset={frame_offset} pid={getattr(self.proc, "pid", None)} '
                        f'poll={self.proc.poll() if self.proc is not None else None}'
                    )
                    if stream_tail:
                        raise RuntimeError(self.partial_frame_error)
