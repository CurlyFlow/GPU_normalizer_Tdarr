...
class AsyncEncodeWriter:
    def __init__(self, handle, *, max_items, zero_copy=False, growable_zero_copy=False, buffer_bytes=0, pool_items=None, writev=False, writev_max_bytes=0, writev_max_items=0, os_write=False):
        self.handle = handle
        self.zero_copy = bool(zero_copy)
        self.growable_zero_copy = bool(growable_zero_copy and zero_copy)
        self.writev = bool(writev)
        self.os_write = bool(os_write and not self.writev)
        ...
        if self.writev and hasattr(os, 'sysconf'):
            try:
                ...
                self.writev_iov_max = 1024
        self.fd = None
        if self.writev or self.os_write:
            try:
                self.fd = handle.fileno()
            except (AttributeError, OSError):
                self.os_write = False
        queue_items = max(1, int(max_items if self.growable_zero_copy else (pool_items if pool_items is not None else max_items)))
        self.queue = queue.Queue(maxsize=queue_items)
        self.free_buffers = queue.Queue(maxsize=queue_items) if self.zero_copy else None
        self.buffer_bytes = max(1, int(buffer_bytes)) if self.zero_copy else 0
        self.pool_limit = queue_items
        self.allocated_buffers = 0
        self.pool_lock = threading.Lock()
        if self.zero_copy:
            if not self.growable_zero_copy:
                for _ in range(queue_items):
                    self.free_buffers.put(self._allocate_buffer(self.buffer_bytes))
        self.closed = False
        self.aborted = False
        self.error = None
        ...
        self.cuda_event_writes = 0
        self.cuda_event_wait_time = 0.0
    ...
    def _is_cuda_event_item(self, item):
        return isinstance(item, tuple) and len(item) == 6 and item[0] == 'cuda_event_borrowed'

    def _allocate_buffer(self, size):
        buf = bytearray(max(self.buffer_bytes, int(size)))
        ptr = ctypes.addressof(ctypes.c_char.from_buffer(buf))
        self.allocated_buffers += 1
        return buf, ptr

    def _release_item(self, item):
        if self._is_cuda_event_item(item):
            _, cuda, context, event, _, release = item
            try:
                self._destroy_cuda_event(cuda, context, event)
            finally:
                release()
        elif self._is_borrowed_item(item):
            item[2]()
        elif self.zero_copy:
    ...
    def _write_all(self, payload):
        ...
        while len(view) > 0:
            ...
            if wrote <= 0:
                raise BrokenPipeError('encode pipe os.write wrote no bytes')
            view = view[wrote:]
    ...
    def _push_cuda_context(self, cuda, context):
        if context is None or not getattr(context, 'value', None):
            return False
        if not getattr(cuda, '_has_ctx_push_api', False):
            raise RuntimeError('CUDA context push API unavailable for event writer')
        rc = cuda.cuCtxPushCurrent_v2(context)
        if rc != 0:
            raise RuntimeError(f'cuCtxPushCurrent(event writer) failed: CUDA error {rc}')
        return True

    def _pop_cuda_context(self, cuda):
        popped = ctypes.c_void_p()
        rc = cuda.cuCtxPopCurrent_v2(ctypes.byref(popped))
        if rc != 0:
            raise RuntimeError(f'cuCtxPopCurrent(event writer) failed: CUDA error {rc}')

    def _destroy_cuda_event(self, cuda, context, event):
        if event is None or not getattr(event, 'value', None):
            return
        pushed = False
        try:
            pushed = self._push_cuda_context(cuda, context)
            cuda.cuEventDestroy_v2(event)
            event.value = None
        finally:
            if pushed:
                self._pop_cuda_context(cuda)

    def _write_cuda_event_item(self, item):
        _, cuda, context, event, payload, release = item
        pushed = False
        try:
            pushed = self._push_cuda_context(cuda, context)
            wait_t0 = time.perf_counter()
            rc = cuda.cuEventSynchronize(event)
            self.cuda_event_wait_time += time.perf_counter() - wait_t0
            if rc != 0:
                raise RuntimeError(f'cuEventSynchronize(event writer) failed: CUDA error {rc}')
            t0 = time.perf_counter()
            if self.os_write:
            ...
            self.worker_write_time += time.perf_counter() - t0
            self.cuda_event_writes += 1
        finally:
            try:
                if event is not None and getattr(event, 'value', None):
                    cuda.cuEventDestroy_v2(event)
                    event.value = None
            finally:
                if pushed:
        ...
        try:
            while True:
                ...
                if self.aborted:
                    ...
                    continue
                if self._is_cuda_event_item(item):
                    self._write_cuda_event_item(item)
                    continue
                if self.writev:
    ...
    def acquire_buffer(self, size):
        ...
        if self.growable_zero_copy:
            try:
                buf_info = self.free_buffers.get_nowait()
            except queue.Empty:
                with self.pool_lock:
                    if self.allocated_buffers < self.pool_limit:
                        return self._allocate_buffer(size)
    ...
    def write_cuda_event_borrowed(self, cuda, context, event, data, release):
        if self.closed:
            try:
                self._destroy_cuda_event(cuda, context, event)
            finally:
            ...
            raise ValueError('async encode writer is closed')
        view = memoryview(data)
        self.queue.put(('cuda_event_borrowed', cuda, context, event, view, release))
        self.borrowed_writes += 1
        self.borrowed_bytes += len(view)
        self.peak_queue = max(self.peak_queue, self.queue.qsize())
...
def maybe_async_encode_writer(streaming_io, handle, *, chunk_bytes):
    if getattr(handle, 'loudnorm_null_write', False):
        return handle, None
    ...
    growable_zero_copy = env_flag('LOUDNORM_GPU_GROWABLE_ZERO_COPY_ENCODE_WRITE', False)
    zero_copy = env_flag('LOUDNORM_GPU_ZERO_COPY_ENCODE_WRITE', False) or growable_zero_copy
    ...
    os_write = env_flag('LOUDNORM_GPU_ASYNC_ENCODE_OS_WRITE', False) and not writev
    ...
    writer = AsyncEncodeWriter(
        ...
        zero_copy=zero_copy,
        growable_zero_copy=growable_zero_copy,
