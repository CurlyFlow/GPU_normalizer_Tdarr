...
def derive_input_shape(args, *, input_sample_bytes, streaming_io, stats_cache_only):
    ...
    ingress_rate = int(args.ingress_rate or 0)
    if ingress_rate <= 0:
        ingress_rate = args.rate
    if ingress_rate != args.rate and not streaming_io:
        raise SystemExit('--ingress-rate requires --streaming-io')
    if ingress_rate != args.rate and args.input_format != 'f64le':
        raise SystemExit('--ingress-rate currently requires f64le input')
...
def derive_exact_feature_config(args, *, exact_limiter_active, source_config):
    ...
    emit_first_pass_json = exact_limiter_active and args.input_format == 'f64le' and bool(args.emit_first_pass_json or args.emit_first_pass_input_json)
    ...
    exact_skip_apply_rolling_ring_6ch = exact_parallel_skip_safe_feedback and args.channels == 6 and env_flag('LOUDNORM_GPU_SKIP_APPLY_ROLLING_RING_6CH')
    exact_safe_skip_6ch_single_wrap = exact_parallel_skip_safe_feedback and args.channels == 6 and env_flag('LOUDNORM_GPU_SAFE_SKIP_6CH_SINGLE_WRAP')
    exact_safe_run_scan_apply_6ch = exact_parallel_skip_safe_feedback and env_flag('LOUDNORM_GPU_SAFE_RUN_SCAN_APPLY_6CH')
    try:
        exact_safe_run_scan_apply_6ch_boundary_halo_windows = max(0, int(os.environ.get('LOUDNORM_GPU_SAFE_RUN_SCAN_APPLY_6CH_BOUNDARY_HALO_WINDOWS', '0')))
    except ValueError:
        exact_safe_run_scan_apply_6ch_boundary_halo_windows = 0
    ...
    return SimpleNamespace(
        ...
        exact_skip_apply_rolling_ring_6ch=exact_skip_apply_rolling_ring_6ch,
        exact_safe_skip_6ch_single_wrap=exact_safe_skip_6ch_single_wrap,
        exact_safe_run_scan_apply_6ch=exact_safe_run_scan_apply_6ch,
        exact_safe_run_scan_apply_6ch_boundary_halo_windows=exact_safe_run_scan_apply_6ch_boundary_halo_windows,
...
def derive_chunk_sizing_config(args, *, input_sample_bytes, nbytes, frames_per_window, exact_limiter_active, source_config, limiter_config):
    ...
    if exact_limiter_active:
        ...
        try:
            safe_skip_chunk_windows = max(0, int(os.environ.get('LOUDNORM_GPU_SAFE_SKIP_CHUNK_WINDOWS', '0')))
        except ValueError:
            safe_skip_chunk_windows = 0
        if safe_skip_chunk_windows > 0:
            safe_skip_chunk_bytes = min(nbytes, max(window_bytes, safe_skip_chunk_windows * window_bytes))
            exact_safe_skip_chunk_bytes = max(chunk_bytes, safe_skip_chunk_bytes)
    ...
    exact_apply_capacity_chunk_bytes = max(chunk_bytes, exact_safe_skip_chunk_bytes)
    apply_input_chunk_bytes = exact_apply_capacity_chunk_bytes
    if exact_limiter_active:
        apply_input_chunk_bytes = exact_apply_capacity_chunk_bytes + (max(limiter_config.limiter_lookahead_frames, limiter_config.final_flush_frames) * frame_bytes)
        if source_config.source_faithful_stereo:
            apply_input_chunk_bytes = exact_apply_capacity_chunk_bytes + (max(limiter_config.limiter_lookahead_frames, limiter_config.final_flush_frames, frames_per_window * 30) * frame_bytes)
    ...
    output_capacity_chunk_bytes = exact_apply_capacity_chunk_bytes if exact_limiter_active else chunk_bytes
    output_chunk_bytes = (output_capacity_chunk_bytes // input_sample_bytes) * output_sample_bytes
    return SimpleNamespace(
        ...
        exact_safe_skip_chunk_bytes=exact_safe_skip_chunk_bytes,
