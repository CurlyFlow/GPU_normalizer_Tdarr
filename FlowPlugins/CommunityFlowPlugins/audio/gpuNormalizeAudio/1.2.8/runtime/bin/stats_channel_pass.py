...
from stats_channel_native import native_channel_stats_supported, run_native_channel_stats_pass
...
from stream_ingress_resample import frame_chunk_reader_for
...
def run_channel_stats_pass(ctx, result):
    ...
    if cfg.ingress_rate == args.rate and native_channel_stats_supported(ctx, source_stage_config):
        run_native_channel_stats_pass(ctx, result, source_stage_config)
        return
    ...
    use_checkpoint_replay = bool(
        getattr(cfg, 'exact_channel_stats_checkpoint_replay', False)
        and not use_source_device_energy
        and not use_source_device_input
        and not use_combined_peaks
        and not getattr(cfg, 'exact_channel_stats_no_peaks', False)
    )
    use_channel_warp = bool(
        getattr(cfg, 'exact_channel_stats_warp', False)
        and not use_checkpoint_replay
        and not use_source_device_energy
        and not getattr(cfg, 'exact_channel_stats_no_peaks', False)
    )
    ...
    use_async_parallel_peaks_requested = bool(
        (not use_checkpoint_replay)
        and (not use_combined_peaks)
        and getattr(cfg, 'exact_stats_async_parallel_peaks', False)
        and cuda_has_async_api
        and hasattr(ctx.cuda, 'cuStreamWaitEvent')
    )
    use_parallel_peaks = bool((not use_checkpoint_replay) and (not use_combined_peaks) and cfg.exact_stats_parallel_peaks and cuda_has_async_api and not use_async_parallel_peaks_requested)
    use_pinned_async_pipeline = bool(
        getattr(cfg, 'exact_stats_async_pinned_pipeline', False)
        and cuda_has_async_api
        and not use_parallel_peaks
        and not use_combined_peaks
        and ctx.buffers.d_stats_in_b.value
    )
    use_async_pipeline = use_pinned_async_pipeline or bool(
    ...
    use_async_parallel_peaks = bool(use_async_pipeline and use_async_parallel_peaks_requested)
    stats_streams = []
    stats_peak_streams = []
    stats_h2d_events = []
    stats_pending = []
    ...
    stats_pinned_ptrs = []
    stats_pinned_raws = []
    stats_prefetch_buffers = []
    stats_payload_releasers = [None, None]
    last_stats_stream_idx = None
    use_direct_pinned_prefetch = bool(
        use_pinned_async_pipeline
        and getattr(cfg, 'stats_prefetch_pinned_async_h2d', False)
    )
    direct_stats_bytearray_payload = bool(getattr(cfg, 'stats_prefetch_bytearray_direct_h2d', False) and not use_async_pipeline)
    direct_stats_bytearray_payload = direct_stats_bytearray_payload or use_direct_pinned_prefetch
    ...
    def release_stats_payload(idx):
        if idx < len(stats_pending) and stats_pending[idx]:
            return
        if idx < len(stats_peak_pending) and stats_peak_pending[idx]:
            return
        releaser = stats_payload_releasers[idx]
        if releaser is None:
            return
        releaser()
        stats_payload_releasers[idx] = None

    def sync_async_peak(idx, label):
        if not use_async_parallel_peaks or not stats_peak_pending[idx]:
            return
        sync_t0 = time.perf_counter()
        chk(ctx.cuda.cuStreamSynchronize(stats_peak_streams[idx]), f'cuStreamSynchronize({label} peak {idx})')
        result.stats_sums_wall_time += time.perf_counter() - sync_t0
        stats_peak_pending[idx] = False

    def sync_async_stats(label):
        if not use_async_pipeline:
            return
        for idx, stream in enumerate(stats_streams):
            if not stats_pending[idx] and not (use_async_parallel_peaks and stats_peak_pending[idx]):
                continue
            if stats_pending[idx]:
                sync_t0 = time.perf_counter()
                chk(ctx.cuda.cuStreamSynchronize(stream), f'cuStreamSynchronize({label} {idx})')
                result.stats_sums_wall_time += time.perf_counter() - sync_t0
                stats_pending[idx] = False
            sync_async_peak(idx, label)
            release_stats_payload(idx)
    ...
    if use_parallel_peaks:
        ...
        stats_pinned_bytes = max(1, int(getattr(cfg, 'stats_chunk_bytes', len(host_in)))) if use_direct_pinned_prefetch else len(host_in)
        for idx in range(2):
            ...
            if use_async_parallel_peaks:
                peak = ctypes.c_void_p()
                event = ctypes.c_void_p()
                chk(ctx.cuda.cuStreamCreate(ctypes.byref(peak), 0), f'cuStreamCreate(stats async peak {idx})')
                chk(ctx.cuda.cuEventCreate(ctypes.byref(event), 0), f'cuEventCreate(stats async h2d {idx})')
                stats_peak_streams.append(peak)
                stats_h2d_events.append(event)
                stats_peak_pending.append(False)
            if use_pinned_async_pipeline:
                ptr, raw, view = alloc_pinned_host_buffer(ctx.cuda, stats_pinned_bytes, f'stats async input {idx}')
                stats_pinned_ptrs.append(ptr)
                stats_pinned_raws.append(raw)
                stats_host_buffers.append(view)
                stats_host_ptrs.append(ptr.value)
                if use_direct_pinned_prefetch:
                    stats_prefetch_buffers.append((view, ptr.value))
            else:
                stage_buffer = bytearray(stats_pinned_bytes)
                stats_host_buffers.append(stage_buffer)
                stats_host_ptrs.append(ctypes.addressof(ctypes.c_char.from_buffer(stage_buffer)))
        if use_direct_pinned_prefetch:
            target_prefetch_buffers = max(3, int(getattr(cfg, 'stats_prefetch_pinned_async_buffers', 2)))
            for idx in range(len(stats_prefetch_buffers), target_prefetch_buffers):
                ptr, raw, view = alloc_pinned_host_buffer(ctx.cuda, stats_pinned_bytes, f'stats async prefetch input {idx}')
                stats_pinned_ptrs.append(ptr)
                stats_pinned_raws.append(raw)
                stats_prefetch_buffers.append((view, ptr.value))
    try:
        with frame_chunk_reader_for(
            ...
            direct_payload_buffers=stats_prefetch_buffers if use_direct_pinned_prefetch else None,
            auto_return_payload=not use_direct_pinned_prefetch,
        ) as chunks:
            chunk_index = 0
            for chunk in chunks:
                async_slot = chunk_index & 1
                if use_async_pipeline and (stats_pending[async_slot] or (use_async_parallel_peaks and stats_peak_pending[async_slot])):
                    if stats_pending[async_slot]:
                        sync_t0 = time.perf_counter()
                    ...
                    sync_async_peak(async_slot, 'stats async reuse')
                    release_stats_payload(async_slot)
                ...
                if use_checkpoint_replay:
                    if not ctx.buffers.d_checkpoint_states.value:
                        raise RuntimeError('checkpoint replay stats requested without checkpoint state buffer')
                    if chunk.frame_offset % cfg.frames_per_window:
                        raise RuntimeError(f'checkpoint replay stats requires window-aligned chunks: frame_offset={chunk.frame_offset}')
                    checkpoint_args = kernel_args.build_channel_checkpoint_args(
                        input_arg,
                        ctx.bindings.peaks_arg,
                        ctx.bindings.q_states_arg,
                        ctx.bindings.checkpoint_states_arg,
                        ctx.bindings.source_start_states_arg,
                        frames_arg,
                        ctx.bindings.channels_arg,
                        ctx.bindings.frame_window_arg,
                    ...
                    replay_args = kernel_args.build_sums_args(
                        input_arg,
                        ctx.bindings.start_states_arg,
                        ctx.bindings.checkpoint_states_arg,
                        frames_arg,
                    ...
                    replay_pairs = windows_this * args.channels
                    replay_grid = min(65535, max(1, (replay_pairs + 255) // 256))
                if use_source_device_energy:
                    channel_kernel_fn = ctx.kernels.channel_sums_source_energy_fn
                elif use_checkpoint_replay:
                    channel_kernel_fn = ctx.kernels.prefix_channel_sums_fn
                elif use_channel_warp:
                    channel_kernel_fn = ctx.kernels.channel_sums_warp_fn
                elif getattr(cfg, 'exact_channel_stats_unroll4', False) and not use_combined_peaks:
                    channel_kernel_fn = ctx.kernels.channel_sums_unroll4_fn
                elif getattr(cfg, 'exact_channel_stats_no_peaks', False) and not use_combined_peaks:
                    channel_kernel_fn = ctx.kernels.channel_sums_no_peaks_fn
                else:
                ...
                if use_async_pipeline:
                    ...
                    if use_direct_pinned_prefetch and getattr(chunk, 'payload_ptr', 0):
                        stats_payload_releasers[async_slot] = chunk.payload_releaser
                        h2d_input_ptr = chunk.payload_ptr
                    else:
                        memoryview(stats_host_buffers[async_slot])[:chunk.this_bytes] = memoryview(host_in)[:chunk.this_bytes]
                        h2d_input_ptr = stats_host_ptrs[async_slot]
                    chk(ctx.cuda.cuMemcpyHtoDAsync_v2(input_ptr, ctypes.c_void_p(h2d_input_ptr), chunk.this_bytes, stream), 'cuMemcpyHtoDAsync(channel stats input)')
                else:
                ...
                if use_parallel_peaks:
                    ...
                    chk(ctx.cuda.cuLaunchKernel(channel_kernel_fn, channel_grid_x, 1, 1, channel_block_x, 1, 1, 0, channel_stream, channel_args, None), 'cuLaunchKernel(channel-stats parallel)')
                    ...
                    if last_stats_stream_idx is not None and stats_pending[last_stats_stream_idx]:
                        ...
                        release_stats_payload(last_stats_stream_idx)
                    if use_checkpoint_replay:
                        launch_kernel(ctx.cuda, 'channel-checkpoint-states-async', ctx.kernels.channel_checkpoint_states_fn, checkpoint_args, grid_x=args.channels, block_x=1, stream=stats_streams[async_slot])
                        launch_kernel(ctx.cuda, 'channel-replay-sums-async', channel_kernel_fn, replay_args, grid_x=replay_grid, block_x=256, stream=stats_streams[async_slot])
                    else:
                        if use_async_parallel_peaks:
                            chk(ctx.cuda.cuEventRecord(stats_h2d_events[async_slot], stats_streams[async_slot]), f'cuEventRecord(stats async h2d {async_slot})')
                            chk(ctx.cuda.cuStreamWaitEvent(stats_peak_streams[async_slot], stats_h2d_events[async_slot], 0), f'cuStreamWaitEvent(stats async peak {async_slot})')
                            launch_kernel(ctx.cuda, 'window-peaks-async-parallel', ctx.kernels.peak_fn, peak_args, grid_x=peak_grid, block_x=256, stream=stats_peak_streams[async_slot])
                            stats_peak_pending[async_slot] = True
                        else:
                        ...
                        launch_kernel(ctx.cuda, 'channel-stats-async', channel_kernel_fn, channel_args, grid_x=channel_grid_x, block_x=channel_block_x, stream=stats_streams[async_slot])
                    ...
                    kernel_t0 = time.perf_counter()
                    if use_checkpoint_replay:
                        launch_kernel(ctx.cuda, 'channel-checkpoint-states-fast', ctx.kernels.channel_checkpoint_states_fn, checkpoint_args, grid_x=args.channels, block_x=1)
                        launch_kernel(ctx.cuda, 'channel-replay-sums-fast', channel_kernel_fn, replay_args, grid_x=replay_grid, block_x=256)
                    elif not use_combined_peaks:
                        launch_kernel(ctx.cuda, 'window-peaks-fast', ctx.kernels.peak_fn, peak_args, grid_x=peak_grid, block_x=256)
                        launch_kernel(ctx.cuda, 'channel-stats-fast', channel_kernel_fn, channel_args, grid_x=channel_grid_x, block_x=channel_block_x)
                    else:
                        launch_kernel(ctx.cuda, 'channel-stats-fast', channel_kernel_fn, channel_args, grid_x=channel_grid_x, block_x=channel_block_x)
                    kernel_dt = time.perf_counter() - kernel_t0
                else:
                    if use_checkpoint_replay:
                        peak_dt = launch_timed_kernel(ctx.cuda, 'channel-checkpoint-states', ctx.kernels.channel_checkpoint_states_fn, checkpoint_args, grid_x=args.channels, block_x=1)
                        channel_dt = launch_timed_kernel(ctx.cuda, 'channel-replay-sums', channel_kernel_fn, replay_args, grid_x=replay_grid, block_x=256)
                    else:
                        ...
                        channel_dt = launch_timed_kernel(ctx.cuda, 'channel-stats', channel_kernel_fn, channel_args, grid_x=channel_grid_x, block_x=channel_block_x)
        ...
        sync_async_stats('stats async cleanup')
        ...
        for stream in stats_peak_streams:
            if stream.value:
                ctx.cuda.cuStreamDestroy_v2(stream)
        for event in stats_h2d_events:
            if event.value:
                ctx.cuda.cuEventDestroy_v2(event)
        for ptr in stats_pinned_ptrs:
            if ptr.value:
                ctx.cuda.cuMemFreeHost(ptr)
