...
def _full_windows_for_first_pass(cfg):
    if not env_flag('LOUDNORM_GPU_FIRST_PASS_FFMPEG_WINDOW_TRIM'):
        return None
    return cfg.total_frames // cfg.frames_per_window
...
def emit_first_pass_json(cuda, args, cfg, buffers, apply_result):
    ...
    output_sums = getattr(cfg, '_first_pass_output_sums_host', None)
    ...
    full_windows = _full_windows_for_first_pass(cfg)
    if input_metrics is None:
        input_i, input_thresh = input_loudness_threshold_from_window_sums(input_sums, cfg.frames_per_window, full_windows=full_windows)
        input_lra = input_loudness_range_from_window_sums(input_sums, cfg.frames_per_window, full_windows=full_windows)
    ...
    replay_input_bias_db = _env_float('LOUDNORM_GPU_FIRST_PASS_REPLAY_INPUT_BIAS_DB', 0.0) if input_metrics is not None else 0.0
    if replay_input_bias_db:
        input_i += replay_input_bias_db
        input_thresh += replay_input_bias_db

    output_i, output_thresh = input_loudness_threshold_from_window_sums(output_sums, cfg.frames_per_window, full_windows=full_windows)
    output_lra = input_loudness_range_from_window_sums(output_sums, cfg.frames_per_window, full_windows=full_windows)
    ...
    print(format_profile_stage('first_pass_json', [
        ...
        ('full_windows', full_windows if full_windows is not None else -1),
        ('input_i', f'{input_i:.6f}'),
        ('input_thresh', f'{input_thresh:.6f}'),
        ('input_lra', f'{input_lra:.6f}'),
        ('input_tp', f'{input_tp:.6f}'),
        ('replay_input_bias_db', f'{replay_input_bias_db:.6f}'),
...
def emit_first_pass_input_json(cuda, args, cfg, buffers):
    if not cfg.emit_first_pass_json:
        return None
    t0 = time.perf_counter()
    input_metrics = getattr(cfg, '_ffmpeg_input_metric_values', None)
    ...
    print(json.dumps(values, indent=4), file=sys.stderr)
    print(format_profile_stage('first_pass_input_json', [
        ('input_metric_source', input_metric_source),
        ('full_windows', full_windows if full_windows is not None else -1),
        ('input_i', f'{input_i:.6f}'),
        ('input_thresh', f'{input_thresh:.6f}'),
