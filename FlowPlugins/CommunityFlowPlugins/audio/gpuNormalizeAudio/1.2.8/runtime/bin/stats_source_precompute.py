...
def run_source_exact_precompute_pass(ctx, result):
    ...
    if cfg.source_precompute_from_channel_sums:
        ...
        write_short_arg = ctypes.c_uint32(0 if (cfg.source_channel_short_exact or cfg.source_channel_short_boundary or getattr(cfg, 'source_channel_short_raw_boundary', False)) else 1)
        ...
        if not needs_source_corrections:
            ...
            return
        needs_source_energy = bool(
            cfg.source_channel_hist4_exact
            or cfg.source_channel_short_exact
            or cfg.source_channel_short_boundary
        )
        if needs_source_energy and not ctx.buffers.d_source_energy.value:
            raise RuntimeError('source exact channel-sum correction energy buffer is not allocated')
        if not ctx.buffers.d_source_start_states.value:
            raise RuntimeError('source exact channel-sum correction buffers are not allocated')
        if getattr(cfg, 'source_channel_short_raw_boundary', False) and not ctx.buffers.d_source_correction_flags.value:
            raise RuntimeError('source exact channel-sum raw correction flags buffer is not allocated')
...
def _run_source_exact_precompute_stage(ctx, result, stage, stage_start_frame, next_window, ready_windows, *, device_energy_end_frame=None, device_input_end_frame=None, stage_active_bytes=None):
    ...
    if ctx.cfg.source_channel_hist4_exact or ctx.cfg.source_channel_short_exact or ctx.cfg.source_channel_short_boundary:
        ...
        if ctx.cfg.source_energy_packed:
            ...
            energy_dt += _launch_source_precompute_kernel(ctx, energy_label, energy_fn, energy_args, grid_x=energy_grid, block_x=32)
        else:
    ...
    if ctx.cfg.source_channel_short_exact:
    ...
    elif getattr(ctx.cfg, 'source_channel_short_raw_boundary', False):
        mark_args = kernel_args.build_source_mark_short_raw_corrections_args(
            ctx.bindings.source_exact_sums_arg,
            ctx.bindings.source_correction_flags_arg,
            ctx.bindings.start_states_arg,
            ctx.bindings.hist_energies_arg,
            ctx.bindings.hist_boundaries_arg,
            target_start_arg,
        ...
        source_exact_dt += launch_timed_kernel(ctx.cuda, 'source-mark-short-raw-corrections', ctx.kernels.source_mark_short_raw_corrections_fn, mark_args, grid_x=1, block_x=1)
        apply_args = kernel_args.build_source_apply_short_raw_corrections_args(
            ctx.bindings.in_arg,
            ctx.bindings.source_start_states_arg,
            ctx.bindings.source_exact_sums_arg,
            ctx.bindings.source_correction_flags_arg,
        ...
        source_exact_dt += launch_timed_kernel(ctx.cuda, 'source-apply-short-raw-corrections', ctx.kernels.source_apply_short_raw_corrections_fn, apply_args, grid_x=grid_windows, block_x=128)
    ...
    if (not ctx.cfg.source_channel_hist4_exact) and ctx.cfg.source_block_sum_candidate:
        ...
        exact_after_window = getattr(ctx.cfg, 'source_block_exact_after_window', -1)
        if exact_after_window >= 0:
            exact_start_window = max(next_window, exact_after_window)
            exact_until_window = getattr(ctx.cfg, 'source_block_exact_until_window', -1)
            exact_end_window = ready_windows
            if exact_until_window >= 0:
                exact_end_window = min(exact_end_window, exact_until_window)
            if exact_start_window < exact_end_window:
                exact_target_start_arg = ctypes.c_uint32(exact_start_window)
                exact_target_windows_arg = ctypes.c_uint32(exact_end_window - exact_start_window)
                exact_after_args = kernel_args.build_source_exact_sums_args(
                    ctx.bindings.source_energy_arg,
                    ctx.bindings.source_exact_sums_arg,
                    ctx.bindings.source_start_states_arg,
                    ctx.bindings.total_frames_arg,
                    input_base_arg,
                    input_frames_arg,
                    exact_target_start_arg,
                    exact_target_windows_arg,
                    ctx.bindings.channels_arg,
                    ctx.bindings.frame_window_arg,
                    ctx.bindings.windows_arg,
                    ctx.bindings.b_arg,
                ...
                exact_pairs = (exact_end_window - exact_start_window) * 2
                exact_block_x = getattr(ctx.cfg, 'source_exact_sums_block_x', 128)
                exact_grid = min(65535, max(1, (exact_pairs + exact_block_x - 1) // exact_block_x))
                if getattr(ctx.cfg, 'source_exact_split_unroll4', False):
                    exact_after_fn = ctx.kernels.source_exact_split_unroll4_sums_fn
                    exact_after_label = 'source-block-exact-after-split-unroll4'
                elif getattr(ctx.cfg, 'source_exact_split_pointer_walk', False):
                    exact_after_fn = ctx.kernels.source_exact_split_pointer_sums_fn
                    exact_after_label = 'source-block-exact-after-split-pointer'
                elif getattr(ctx.cfg, 'source_exact_split_readonly_loads', False):
                    exact_after_fn = ctx.kernels.source_exact_split_readonly_sums_fn
                    exact_after_label = 'source-block-exact-after-split-readonly'
                elif ctx.cfg.source_exact_split_sums:
                    exact_after_fn = ctx.kernels.source_exact_split_sums_fn
                    exact_after_label = 'source-block-exact-after-split'
                else:
                    exact_after_fn = ctx.kernels.source_exact_sums_fn
                    exact_after_label = 'source-block-exact-after'
                source_exact_dt += _launch_source_precompute_kernel(ctx, exact_after_label, exact_after_fn, exact_after_args, grid_x=exact_grid, block_x=exact_block_x)
