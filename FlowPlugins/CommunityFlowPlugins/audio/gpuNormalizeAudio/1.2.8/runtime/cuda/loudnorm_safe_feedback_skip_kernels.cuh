__device__ __forceinline__ unsigned int safe_skip_wrap_frame(
    unsigned int frame,
    unsigned int limiter_lookahead_frames,
    int single_wrap
) {
    if (single_wrap) {
        return frame >= limiter_lookahead_frames ? frame - limiter_lookahead_frames : frame;
    }
    while (frame >= limiter_lookahead_frames) frame -= limiter_lookahead_frames;
    return frame;
}
...
extern "C" __global__ void safe_feedback_skip_apply6_f64_kernel(
    ...
    (void)output_window_sums;
    if (blockIdx.x != 0 || (channels != 6U && channels != 2U)) return;
    const unsigned int tid = threadIdx.x;
    const unsigned int stride = blockDim.x;
    const unsigned int source_feedback_mode = source_faithful_stereo & 15U;
    const int frame_specialized = ((source_faithful_stereo & 16U) != 0U);
    const int rolling_ring_6ch = ((source_faithful_stereo & 32U) != 0U) && channels == 6U;
    const int skip_limiter_lookahead_scan_6ch = ((source_faithful_stereo & 64U) != 0U) && channels == 6U;
    const int single_wrap_6ch = ((source_faithful_stereo & 128U) != 0U) && channels == 6U;
    const int skip_direct_clamp_6ch = ((source_faithful_stereo & 256U) != 0U) && channels == 6U;
    const int skip_fill_peak_scan_6ch = ((source_faithful_stereo & 512U) != 0U) && channels == 6U;
    const int source_precomputed = (source_feedback_mode >= 2U && channels == 2U && source_channel_sums);
    const int skip_limiter_lookahead_scan = ((source_feedback_mode == 3U || source_feedback_mode == 5U) && source_precomputed);
    const int source_fused_direct_feedback = (source_feedback_mode >= 4U && source_precomputed);
    ...
    while (produced_s < output_frames && out_frame_s < total_frames) {
        ...
        if (frame_specialized && above_threshold_s != 0) {
            if (rolling_ring_6ch) {
                unsigned int dst_frame = fill_write_index + tid;
                dst_frame = safe_skip_wrap_frame(dst_frame, limiter_lookahead_frames, single_wrap_6ch);
                for (unsigned int n = tid; n < nb_s; n += stride) {
                    const unsigned int src_frame = fill_write_frame + n;
                    const int src_valid = (src_frame < total_frames && src_frame >= input_base_frame && src_frame < input_base_frame + input_frames);
                    if (!src_valid && src_frame < total_frames) atomicExch(&input_missing_s, 1U);
                    ...
                    const unsigned long long dst_base = (unsigned long long)dst_frame * 6ULL;
                    const unsigned long long src_base = src_valid ? ((unsigned long long)(src_frame - input_base_frame) * 6ULL) : 0ULL;
                    const double v0 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 0ULL] : 0.0, timed_gain), offset_amp);
                    const double v1 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 1ULL] : 0.0, timed_gain), offset_amp);
                    const double v2 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 2ULL] : 0.0, timed_gain), offset_amp);
                    const double v3 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 3ULL] : 0.0, timed_gain), offset_amp);
                    const double v4 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 4ULL] : 0.0, timed_gain), offset_amp);
                    const double v5 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 5ULL] : 0.0, timed_gain), offset_amp);
                    if (!skip_fill_peak_scan_6ch && (v0 > ceiling || v0 < -ceiling || v1 > ceiling || v1 < -ceiling || v2 > ceiling || v2 < -ceiling || v3 > ceiling || v3 < -ceiling || v4 > ceiling || v4 < -ceiling || v5 > ceiling || v5 < -ceiling)) atomicMax(&limiter_maybe_above_s, 3U);
                    limiter_buf[dst_base + 0ULL] = v0;
                    limiter_buf[dst_base + 1ULL] = v1;
                    limiter_buf[dst_base + 2ULL] = v2;
                    limiter_buf[dst_base + 3ULL] = v3;
                    limiter_buf[dst_base + 4ULL] = v4;
                    limiter_buf[dst_base + 5ULL] = v5;
                    dst_frame += stride;
                    dst_frame = safe_skip_wrap_frame(dst_frame, limiter_lookahead_frames, single_wrap_6ch);
                }
            } else {
            for (unsigned int n = tid; n < nb_s; n += stride) {
                ...
                dst_frame = safe_skip_wrap_frame(dst_frame, limiter_lookahead_frames, single_wrap_6ch);
                const unsigned long long dst_base = (unsigned long long)dst_frame * (unsigned long long)channels;
                const unsigned long long src_base = src_valid ? ((unsigned long long)(src_frame - input_base_frame) * (unsigned long long)channels) : 0ULL;
                if (channels == 6U) {
                    const double v0 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 0ULL] : 0.0, timed_gain), offset_amp);
                    const double v1 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 1ULL] : 0.0, timed_gain), offset_amp);
                    const double v2 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 2ULL] : 0.0, timed_gain), offset_amp);
                    ...
                    const double v0 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 0ULL] : 0.0, timed_gain), offset_amp);
                    const double v1 = __dmul_rn(__dmul_rn(src_valid ? input[src_base + 1ULL] : 0.0, timed_gain), offset_amp);
                    if (v0 > ceiling || v0 < -ceiling || v1 > ceiling || v1 < -ceiling) atomicMax(&limiter_maybe_above_s, 3U);
                    limiter_buf[dst_base + 0ULL] = v0;
                    limiter_buf[dst_base + 1ULL] = v1;
                }
            }
            ...
            for (unsigned int idx = tid; idx < fill_samples; idx += stride) {
                ...
                if (!skip_fill_peak_scan_6ch && (v > ceiling || v < -ceiling)) atomicMax(&limiter_maybe_above_s, 3U);
                unsigned int dst_frame = fill_write_index + n;
                dst_frame = safe_skip_wrap_frame(dst_frame, limiter_lookahead_frames, single_wrap_6ch);
        ...
        if (tid == 0U) {
            write_index_s += nb_s;
            write_index_s = safe_skip_wrap_frame(write_index_s, limiter_lookahead_frames, single_wrap_6ch);
            write_frame_s += nb_s;
        ...
        const int skip_lookahead_this = skip_limiter_lookahead_scan || (skip_limiter_lookahead_scan_6ch && above_threshold_s != 0 && limiter_maybe_above_s == 0U && limiter_state_s == 0);
        if (!skip_lookahead_this) {
            if (frame_specialized && above_threshold_s != 0) {
                if (rolling_ring_6ch) {
                unsigned int src_frame = output_limiter_index_s + attack_length + tid;
                src_frame = safe_skip_wrap_frame(src_frame, limiter_lookahead_frames, single_wrap_6ch);
                for (unsigned int n = tid; n < nb_s; n += stride) {
                    const unsigned long long base = (unsigned long long)src_frame * 6ULL;
                    const double v0 = limiter_buf[base + 0ULL];
                    const double v1 = limiter_buf[base + 1ULL];
                    const double v2 = limiter_buf[base + 2ULL];
                    const double v3 = limiter_buf[base + 3ULL];
                    const double v4 = limiter_buf[base + 4ULL];
                    const double v5 = limiter_buf[base + 5ULL];
                    if (v0 > ceiling || v0 < -ceiling || v1 > ceiling || v1 < -ceiling || v2 > ceiling || v2 < -ceiling || v3 > ceiling || v3 < -ceiling || v4 > ceiling || v4 < -ceiling || v5 > ceiling || v5 < -ceiling) atomicMax(&limiter_maybe_above_s, 3U);
                    src_frame += stride;
                    src_frame = safe_skip_wrap_frame(src_frame, limiter_lookahead_frames, single_wrap_6ch);
                }
                } else {
                for (unsigned int n = tid; n < nb_s; n += stride) {
                    ...
                    src_frame = safe_skip_wrap_frame(src_frame, limiter_lookahead_frames, single_wrap_6ch);
                    const unsigned long long base = (unsigned long long)src_frame * (unsigned long long)channels;
                    if (channels == 6U) {
                        const double v0 = limiter_buf[base + 0ULL];
                        const double v1 = limiter_buf[base + 1ULL];
                        const double v2 = limiter_buf[base + 2ULL];
        ...
        if (!use_limiter_s && tid < channels && nb_s > 0U) {
            ...
            prev_frame = safe_skip_wrap_frame(prev_frame, limiter_lookahead_frames, single_wrap_6ch);
        ...
        if (!use_limiter_s) {
            if (frame_specialized && above_threshold_s != 0) {
                if (rolling_ring_6ch) {
                unsigned int src_frame = output_limiter_index_s + tid;
                src_frame = safe_skip_wrap_frame(src_frame, limiter_lookahead_frames, single_wrap_6ch);
                for (unsigned int n = tid; n < nb_s; n += stride) {
                    const unsigned long long src_base = (unsigned long long)src_frame * 6ULL;
                    const unsigned long long dst_base = (unsigned long long)(produced_s + n) * 6ULL;
                    double v0 = limiter_buf[src_base + 0ULL];
                    double v1 = limiter_buf[src_base + 1ULL];
                    double v2 = limiter_buf[src_base + 2ULL];
                    double v3 = limiter_buf[src_base + 3ULL];
                    double v4 = limiter_buf[src_base + 4ULL];
                    double v5 = limiter_buf[src_base + 5ULL];
                    if (!skip_direct_clamp_6ch) {
                    if (v0 > ceiling) v0 = ceiling; else if (v0 < -ceiling) v0 = -ceiling;
                    if (v1 > ceiling) v1 = ceiling; else if (v1 < -ceiling) v1 = -ceiling;
                    if (v2 > ceiling) v2 = ceiling; else if (v2 < -ceiling) v2 = -ceiling;
                    if (v3 > ceiling) v3 = ceiling; else if (v3 < -ceiling) v3 = -ceiling;
                    if (v4 > ceiling) v4 = ceiling; else if (v4 < -ceiling) v4 = -ceiling;
                    if (v5 > ceiling) v5 = ceiling; else if (v5 < -ceiling) v5 = -ceiling;
                    }
                    output[dst_base + 0ULL] = v0;
                    output[dst_base + 1ULL] = v1;
                    output[dst_base + 2ULL] = v2;
                    output[dst_base + 3ULL] = v3;
                    output[dst_base + 4ULL] = v4;
                    output[dst_base + 5ULL] = v5;
                    src_frame += stride;
                    src_frame = safe_skip_wrap_frame(src_frame, limiter_lookahead_frames, single_wrap_6ch);
                }
                } else {
                for (unsigned int n = tid; n < nb_s; n += stride) {
                    ...
                    src_frame = safe_skip_wrap_frame(src_frame, limiter_lookahead_frames, single_wrap_6ch);
                    const unsigned long long src_base = (unsigned long long)src_frame * (unsigned long long)channels;
                    const unsigned long long dst_base = (unsigned long long)(produced_s + n) * (unsigned long long)channels;
                    if (channels == 6U) {
                        double v0 = limiter_buf[src_base + 0ULL];
                        double v1 = limiter_buf[src_base + 1ULL];
                        double v2 = limiter_buf[src_base + 2ULL];
                ...
                for (unsigned int idx = tid; idx < fill_samples; idx += stride) {
                    ...
                    if (!(skip_direct_clamp_6ch && channels == 6U)) {
