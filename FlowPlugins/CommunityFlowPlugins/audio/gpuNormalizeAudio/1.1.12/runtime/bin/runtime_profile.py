...
def emit_runtime_summary(args, cfg, *, gpu_name, elapsed, stats_result, gain_metrics, apply_result, bindings):
    ...
    print(f'audio_seconds={cfg.seconds:.3f} rate={args.rate} channels={args.channels} windows={cfg.windows} pcm_mib={cfg.mib:.1f} chunk_mib={cfg.chunk_bytes/(1024*1024):.1f} input_format={args.input_format} output_format={args.output_format} streaming_io={1 if cfg.streaming_io else 0} source_mode={source_mode} source_precompute_windows={cfg.source_precompute_windows} source_precompute_from_channel_sums={1 if cfg.source_precompute_from_channel_sums else 0} source_precompute_in_stats={1 if cfg.source_precompute_in_stats else 0}', file=sys.stderr)
