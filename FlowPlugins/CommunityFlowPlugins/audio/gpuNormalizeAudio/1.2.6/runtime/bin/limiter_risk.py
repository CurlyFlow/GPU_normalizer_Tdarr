...
def build_prelimiter_risk_run_cache(prelimiter_unsafe_flags, windows):
    unsafe_prefix = [0] * (windows + 1)
    for w in range(windows):
        unsafe_prefix[w + 1] = unsafe_prefix[w] + (1 if prelimiter_unsafe_flags[w] else 0)

    run_end_window = [windows] * windows
    run_start = 0
    while run_start < windows:
        current_flag = prelimiter_unsafe_flags[run_start]
        run_end = run_start + 1
        while run_end < windows and prelimiter_unsafe_flags[run_end] == current_flag:
            run_end += 1
        for w in range(run_start, run_end):
            run_end_window[w] = run_end
        run_start = run_end
    return unsafe_prefix, run_end_window
