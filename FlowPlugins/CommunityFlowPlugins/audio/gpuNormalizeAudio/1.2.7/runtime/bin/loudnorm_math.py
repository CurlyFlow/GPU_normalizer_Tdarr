...
def _window_limit(window_sums, full_windows=None):
    if full_windows is None:
        return len(window_sums)
    return min(len(window_sums), max(0, int(full_windows)))


def input_loudness_from_window_sums(window_sums, frames_per_window, full_windows=None):
    loudness, _threshold = input_loudness_threshold_from_window_sums(window_sums, frames_per_window, full_windows=full_windows)
    return loudness


def input_loudness_threshold_from_window_sums(window_sums, frames_per_window, full_windows=None):
    energies, boundaries = histogram_tables()
    hist = [0] * 1000
    limit = _window_limit(window_sums, full_windows)
...
def input_loudness_range_from_window_sums(window_sums, frames_per_window, full_windows=None):
    energies, boundaries = histogram_tables()
    hist = [0] * 1000
    limit = _window_limit(window_sums, full_windows)
    for t in range(29, limit, 10):
