...
extern "C" __global__ void kweight_window_checkpoint_states_f64_channel_kernel(
    const double *input,
    unsigned int *peak_bits,
    double *states,
    double *checkpoint_states,
    ...
    const unsigned int c = blockIdx.x;
    if (c >= channels || threadIdx.x != 0 || frames_per_window == 0U) return;
    int slot = -1;
    if (channels == 4) {
        if (c == 0) slot = 0;
        else if (c == 1) slot = 1;
    ...
    if (slot < 0) {
        ...
        for (unsigned int i = 0; i < frames; i++) {
            ...
            const unsigned int bits = __float_as_uint((float)fabs(input[(unsigned long long)i * channels + c]));
    ...
    if (frames > 0U) {
        ...
        checkpoint_states[state_base + 0] = v1;
        checkpoint_states[state_base + 1] = v2;
        checkpoint_states[state_base + 2] = v3;
        checkpoint_states[state_base + 3] = v4;
    ...
    if (frames > 0U && peak_bits && window_peak_bits != 0U) atomicMax(&peak_bits[current_w], window_peak_bits);
    states[c * 4 + 0] = v1;
    states[c * 4 + 1] = v2;
    states[c * 4 + 2] = v3;
    states[c * 4 + 3] = v4;
...
extern "C" __global__ void kweight_window_sums_f64_channel_unroll4_kernel(
    const double *__restrict__ input,
    double *__restrict__ channel_sums,
    unsigned int *__restrict__ peak_bits,
    double *__restrict__ states,
    double *__restrict__ source_start_states,
    unsigned int frames,
    unsigned int channels,
    unsigned int frames_per_window,
    unsigned int global_frame_offset,
    ...
    if (slot < 0) {
        ...
        if (!LOUDNORM_EXACT_GENERIC_LIMITER && (global_frame_offset % frames_per_window) == 0U) {
            ...
            for (unsigned int local_w = 0; local_w < local_windows; local_w++) {
                ...
                if (end > frames) end = frames;
                unsigned int window_peak_bits = 0U;
                const double *sample_ptr = input + ((unsigned long long)start * channels) + c;
                unsigned int i = start;
                for (; i + 4U <= end; i += 4U) {
                    unsigned int bits0 = __float_as_uint((float)fabs(sample_ptr[0]));
                    unsigned int bits1 = __float_as_uint((float)fabs(sample_ptr[channels]));
                    unsigned int bits2 = __float_as_uint((float)fabs(sample_ptr[(unsigned long long)channels * 2ULL]));
                    unsigned int bits3 = __float_as_uint((float)fabs(sample_ptr[(unsigned long long)channels * 3ULL]));
                    if (bits0 > window_peak_bits) window_peak_bits = bits0;
                    if (bits1 > window_peak_bits) window_peak_bits = bits1;
                    if (bits2 > window_peak_bits) window_peak_bits = bits2;
                    if (bits3 > window_peak_bits) window_peak_bits = bits3;
                    sample_ptr += (unsigned long long)channels * 4ULL;
                }
                for (; i < end; i++) {
                    const unsigned int bits = __float_as_uint((float)fabs(*sample_ptr));
                    if (bits > window_peak_bits) window_peak_bits = bits;
                    sample_ptr += channels;
                }
                if (window_peak_bits != 0U) atomicMax(&peak_bits[first_w + local_w], window_peak_bits);
...
#define LOUDNORM_CHANNEL_STATS_UNROLL4_STEP() \
    do { \
        const double x = *sample_ptr; \
        if (peak_bits) { \
            const unsigned int bits = __float_as_uint((float)fabs(x)); \
            if (bits > window_peak_bits) window_peak_bits = bits; \
        } \
    ...
    if (!LOUDNORM_EXACT_GENERIC_LIMITER && (channels == 6U || channels == 2U) && (global_frame_offset % frames_per_window) == 0U) {
        ...
        for (unsigned int local_w = 0; local_w < local_windows; local_w++) {
            ...
            unsigned int i = start;
            for (; i + 4U <= end; i += 4U) {
                LOUDNORM_CHANNEL_STATS_UNROLL4_STEP();
                LOUDNORM_CHANNEL_STATS_UNROLL4_STEP();
                LOUDNORM_CHANNEL_STATS_UNROLL4_STEP();
                LOUDNORM_CHANNEL_STATS_UNROLL4_STEP();
            }
        ...
        states[c * 4 + 2] = v3;
        states[c * 4 + 3] = v4;
#undef LOUDNORM_CHANNEL_STATS_UNROLL4_STEP
        return;
    }
...
extern "C" __global__ void kweight_window_sums_f64_channel_warp_kernel(
    const double *input,
    double *channel_sums,
    unsigned int *peak_bits,
    double *states,
...
extern "C" __global__ void kweight_window_sums_f64_channel_no_peaks_kernel(
    const double *input,
    double *channel_sums,
    unsigned int *peak_bits,
    double *states,
