...
function pairedStatsErrorPaths({ fusePairedCpuMeasure, partner, partnerStatsErr, plan, rawTeeCpuMeasure, statsErr }) {
  return [
    statsErr,
    partnerStatsErr,
    fusePairedCpuMeasure ? plan.measureErr : null,
    rawTeeCpuMeasure ? plan.measureErr : null,
    rawTeeCpuMeasure ? partner.measureErr : null,
  ].filter(Boolean);
}
...
function createPairedSplitStatsProcessGraph({
  ...
  if (rawTeeCpuMeasure) {
    const fifos = [fallbackFifo, originalFifo, fallbackRelayFifo, originalRelayFifo, fallbackCpuFifo, originalCpuFifo];
    return {
      backgrounds: [
        { command: fallbackStatsPlan, pidVar: "pid_fb", teePaths: [statsErr] },
        { command: originalStatsPlan, pidVar: "pid_orig", teePaths: [partnerStatsErr] },
        { command: fallbackCpuMeasureCommand, pidVar: "pid_fb_cpu" },
        { command: originalCpuMeasureCommand, pidVar: "pid_orig_cpu" },
        { command: fallbackRelayCommand, pidVar: "pid_fb_relay" },
        { command: originalRelayCommand, pidVar: "pid_orig_relay" },
      ],
      checks: [
      ...
      foreground: ffmpegStatsDecode,
      foregroundCodeVar: "ffmpeg_code",
      foregroundFailurePidVars: ["pid_fb", "pid_orig", "pid_fb_cpu", "pid_orig_cpu", "pid_fb_relay", "pid_orig_relay"],
      pipefail: false,
      removePaths: [...fifos, ...errorPaths],
      waits: [
        { codeVar: "fb_relay_code", pidVar: "pid_fb_relay" },
        ...
        { codeVar: "fb_code", pidVar: "pid_fb" },
        { codeVar: "orig_code", pidVar: "pid_orig" },
        { codeVar: "fb_cpu_code", pidVar: "pid_fb_cpu" },
        { codeVar: "orig_cpu_code", pidVar: "pid_orig_cpu" },
      ],
    };
  }
