...
def build_prelimiter_risk_map(peak_bits_host, gains_host, gains_next_host, windows, ceiling, expand_windows):
    ...
    for w in range(windows):
        ...
        if pre_peak > ceiling:
            ...
            if first_unsafe < 0:
                first_unsafe = w
            last_unsafe = w
            lo = max(0, w - expand_windows)
            hi = min(windows, w + expand_windows + 1)
