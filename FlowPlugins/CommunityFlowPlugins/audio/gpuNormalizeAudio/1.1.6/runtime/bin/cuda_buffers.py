...
def allocate_runtime_buffers(cuda, args, windows, chunk_bytes, apply_input_chunk_bytes, output_chunk_bytes, limiter_alloc_frames, exact_limiter_active, exact_segmented_safe_feedback=False):
    ...
    if exact_limiter_active:
        chk(cuda.cuMemAlloc_v2(ctypes.byref(buffers.d_limiter_buf), limiter_alloc_frames * args.channels * 8), 'cuMemAlloc(limiter_buf)')
        ...
        if exact_segmented_safe_feedback:
