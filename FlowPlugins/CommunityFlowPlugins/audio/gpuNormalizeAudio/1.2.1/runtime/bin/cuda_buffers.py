...
def allocate_runtime_buffers(cuda, args, cfg):
    ...
    chk(cuda.cuMemAlloc_v2(ctypes.byref(buffers.d_stats_in_b), stats_chunk_bytes), 'cuMemAlloc(stats input b)')
    ...
    if cfg.exact_limiter_active:
        ...
        if cfg.source_exact_precompute:
            ...
            if (not cfg.source_precompute_from_channel_sums) or cfg.source_channel_hist4_exact or cfg.source_channel_hist4_boundary or cfg.source_channel_short_exact or cfg.source_channel_short_boundary:
                chk(cuda.cuMemAlloc_v2(ctypes.byref(buffers.d_source_start_states), state_bytes), 'cuMemAlloc(source_start_states)')
            if (not cfg.source_precompute_from_channel_sums) or cfg.source_channel_hist4_exact or cfg.source_channel_short_exact or cfg.source_channel_short_boundary:
        ...
        if cfg.audit_ffmpeg_input_metrics or cfg.emit_first_pass_json:
            ...
            chk(cuda.cuMemAlloc_v2(ctypes.byref(buffers.d_input_metric_audit_sums), audit_windows * 8), 'cuMemAlloc(input_metric_audit_sums)')
