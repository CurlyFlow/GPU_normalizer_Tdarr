...
def _launch_apply_kernel(ctx, profile_label, kernel_fn, launch_args, *, grid_x=1, block_x=1, check_label=None):
...
def prefill_exact_output(ctx, result, feedback_state_i_host, *, frame_offset, this_samples, this_frames, input_base_arg, exact_chunk_unsafe, skip_safe_fill_chunk):
    ...
    if not chunk_unsafe and cfg.exact_use_prefilled_output:
        ...
        prefill_dt = _launch_apply_kernel(ctx, 'apply-prefill', ctx.kernels.prefill_apply_fn, prefill_args, grid_x=prefill_grid, block_x=256, check_label='cuLaunchKernel(apply prefill exact output)')
    ...
    feedback_state_i_host[FB_I_SKIP_SAFE_FEEDBACK] = 1 if ((not chunk_unsafe and cfg.exact_skip_safe_feedback) or (chunk_unsafe and cfg.exact_skip_unsafe_feedback)) else 0
...
def _can_parallel_skip_safe_feedback(ctx, apply_chunk, feedback_state_i_host):
    cfg = ctx.cfg
    args = ctx.args
    threshold_ready = feedback_state_i_host[FB_I_ABOVE_THRESHOLD] != 0
...
def _source_safe_feedback_args(ctx):
    if not ctx.cfg.source_exact_precompute:
        return ctypes.c_uint64(0), ctypes.c_uint32(0)
    source_mode = 3 if ctx.cfg.source_skip_limiter_lookahead_scan else 2
    return ctx.bindings.source_exact_sums_arg, ctypes.c_uint32(source_mode)


def _reused_safe_feedback_apply6_args(ctx, apply_chunk):
    if not ctx.cfg.exact_apply_arg_reuse:
        return None
...
def _launch_source_safe_run_scan_apply(ctx, result, apply_chunk, feedback_state_i_host, *, local_windows):
    ...
    fill_grid = min(65535, max(1, (samples_arg.value + 255) // 256))
    fill_dt = _launch_apply_kernel(ctx, 'source-safe-run-fill', ctx.kernels.safe_feedback_fill_fn, fill_args, grid_x=fill_grid, block_x=256)
    kernel_dt = stitch_dt + fill_dt
    result.apply_kernel_time += kernel_dt
    result.exact_safe_kernel_time += kernel_dt
    result.exact_safe_apply_kernel_time += kernel_dt
...
def _launch_safe_run_scan_apply_6ch(ctx, result, apply_chunk, feedback_state_i_host, *, local_windows):
    ...
    if not (
        ...
        and feedback_state_i_host[FB_I_ABOVE_THRESHOLD] != 0
        and feedback_state_i_host[FB_I_LIMITER_MAYBE_ABOVE_CEILING] == 0
        and feedback_state_i_host[FB_I_OUT_WINDOW_COUNT] == 0
    ):
        return False
    ...
    fill_grid = min(65535, max(1, (samples_arg.value + 255) // 256))
    fill_dt = _launch_apply_kernel(ctx, 'safe-run-fill-6ch', ctx.kernels.safe_feedback_fill_fn, fill_args, grid_x=fill_grid, block_x=256)
    kernel_dt = stitch_dt + fill_dt
    result.apply_kernel_time += kernel_dt
    result.exact_safe_kernel_time += kernel_dt
    result.exact_safe_apply_kernel_time += kernel_dt
...
def launch_sync_apply_kernel(ctx, result, apply_chunk, feedback_state_i_host, *, exact_chunk_unsafe, frame_offset, exact_prefinal_frames):
    ...
    if segmented_safe_feedback_chunk:
        ...
        if _can_parallel_skip_safe_feedback(ctx, apply_chunk, feedback_state_i_host):
            skip_apply_args = _reused_safe_feedback_apply6_args(ctx, apply_chunk)
            if skip_apply_args is None:
                frames_arg = ctypes.c_uint32(apply_chunk.this_frames)
                source_sums_arg, source_flag_arg = _source_safe_feedback_args(ctx)
            ...
            apply_dt = _launch_apply_kernel(ctx, 'safe-feedback-skip-parallel6', ctx.kernels.safe_feedback_skip_apply_fn, skip_apply_args, block_x=256)
        ...
        if cfg.source_exact_precompute:
            ...
            safe_apply_args = _reused_safe_feedback_apply6_args(ctx, apply_chunk)
            if safe_apply_args is None:
                frames_arg = ctypes.c_uint32(apply_chunk.this_frames)
                source_sums_arg, source_flag_arg = _source_safe_feedback_args(ctx)
            ...
            apply_dt = _launch_apply_kernel(ctx, 'safe-feedback-apply6', ctx.kernels.safe_feedback_apply_fn, safe_apply_args, block_x=ctx.args.channels)
            result.apply_kernel_time += apply_dt
            result.exact_safe_kernel_time += apply_dt
            result.exact_safe_apply_kernel_time += apply_dt
    ...
    if (not cfg.source_exact_precompute or feedback_state_i_host[FB_I_ABOVE_THRESHOLD] != 0) and parallel_unsafe_feedback_status(
        ctx.args,
        ...
        local_windows = apply_chunk.this_frames // cfg.frames_per_window
        unsafe_apply_args = _reused_safe_feedback_apply6_args(ctx, apply_chunk)
        if unsafe_apply_args is None:
            frames_arg = ctypes.c_uint32(apply_chunk.this_frames)
            source_sums_arg, source_flag_arg = _source_safe_feedback_args(ctx)
        ...
        if cfg.source_unsafe_skip_kernel and cfg.source_exact_precompute and ctx.args.channels == 2:
            unsafe_kernel = ctx.kernels.safe_feedback_skip_apply_fn
            unsafe_label = 'unsafe-feedback-skip-parallel6'
            unsafe_block_x = 256
        elif cfg.exact_unsafe_skip_kernel_6ch and ctx.args.channels == 6 and feedback_state_i_host[FB_I_ABOVE_THRESHOLD] != 0:
            unsafe_kernel = ctx.kernels.safe_feedback_skip_apply_fn
            unsafe_label = 'unsafe-feedback-skip-parallel6ch'
            unsafe_block_x = 256
        apply_dt = _launch_apply_kernel(ctx, unsafe_label, unsafe_kernel, unsafe_apply_args, block_x=unsafe_block_x)
        result.apply_kernel_time += apply_dt
    ...
    if segmented_safe_feedback_fallback:
        result.segmented_safe_feedback_profile.record_fallback()
    apply_dt = _launch_apply_kernel(ctx, 'apply', ctx.kernels.apply_fn, apply_chunk.apply_args, grid_x=apply_chunk.grid_samples, block_x=1 if cfg.exact_limiter_active else 256, check_label=f'cuLaunchKernel({apply_chunk.kernel_label})')
    result.apply_kernel_time += apply_dt
