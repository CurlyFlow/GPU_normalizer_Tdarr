...
const details = () => ({
  ...
  inputs: [
    ...
    { label: "GPU Chunk MiB", name: "gpuChunkMiB", type: "string", defaultValue: "1", inputUI: { type: "text" }, tooltip: "Input/gain/output chunk size used by the GPU runtime." },
...
const plugin = async (args) => {
  ...
  const startCpuLoudnormTask = (plan, opts = {}) => {
    ...
    const task = (async () => {
      ...
      const result = { values, wallSec: measureRes.wallSec, source: background ? "prefetched" : "measured" };
  ...
  const getCpuLoudnormRecord = async (plan, parseLine) => {
    ...
    if (externalCached) {
      const result = { values: externalCached, wallSec: 0, source: "external_cached" };
      cpuLoudnormResults.set(key, result);
      return result;
    }
    const existingResult = cpuLoudnormResults.get(key);
    if (existingResult) return { ...existingResult, reused: true };
    const task = cpuLoudnormTasks.get(key) || startCpuLoudnormTask(plan, { parseLine });
    const result = await task;
    return result.source === "prefetched" ? { ...result, reused: true } : result;
  };

  const prefetchNextCpuLoudnorm = (afterPlanIdx) => {
    if (!useGpuSourcePort) return;
    for (let idx = afterPlanIdx; idx < audioPlans.length; idx += 1) {
      const nextPlan = audioPlans[idx];
  ...
  for (const plan of audioPlans) {
    ...
    if (useGpuSourcePort) {
      ...
      logProfileStage(args, { scope: "plugin", name: "cpu_loudnorm_first_pass", stream: plan.idx, source_stream: plan.sourceIdx, channels: plan.channels, wall_sec: measureRecord.wallSec, cached: measureRecord.source === "external_cached" || measureRecord.reused ? 1 : 0, prefetched: measureRecord.source === "prefetched" ? 1 : 0 });
      ...
      prefetchNextCpuLoudnorm(plan.idx + 1);
    }
    if (useStreamingSourcePort) {
      const decodeCommand = [
        String(args.ffmpegPath), "-hide_banner", "-nostats", "-nostdin", "-progress", "pipe:2", "-y", "-i", String(plan.sourceInput),
        "-map", `0:a:${plan.sourceAudioIdx}`, "-vn", "-sn", "-dn", ...decodeAudioArgs(plan), "-ar", String(sampleRate), "-f", plan.rawInputFormat, "pipe:1",
      ];
      const encodeCommand = [
    ...
    const decode = [
      q(args.ffmpegPath), "-hide_banner", "-nostats", "-nostdin", "-progress", "pipe:2", "-y", "-i", q(plan.sourceInput),
      "-map", `0:a:${plan.sourceAudioIdx}`, "-vn", "-sn", "-dn", ...decodeAudioArgs(plan).map(q), "-ar", String(sampleRate), "-f", q(plan.rawInputFormat), q(plan.rawInput),
    ].join(" ");
