...
const STEREO_LANGUAGE_ORDER_TOOLTIP = `Comma-separated language priority used to sort audio streams before choosing the source for generated 2-channel fallback. Language order uses source metadata and cannot fix missing/wrong tags. Default/copy: ${DEFAULT_STEREO_LANGUAGE_ORDER}. Copy presets: ${TRACK_ORDER_COPY_PRESETS}. Common codes: ${COMMON_LANGUAGE_CODES}.`;
...
const plugin = async (args) => {
  ...
  const cleanupFilesForPlan = (plan) => [plan.rawInput, plan.gains, ...(plan.statsCaches || [plan.statsCache]), plan.measureErr, ...(plan.statsErrs || [plan.statsErr]), plan.sourceErr, plan.rawGpu, plan.normalizedAudio, plan.fifoInput, plan.fifoOutput];
  ...
  const earlyCpuPrefetch = envFlag("LOUDNORM_GPU_CPU_LOUDNORM_EARLY_PREFETCH", true);
  ...
  const startSplitStatsTask = (plan, reason, parseLine = () => {}, cpuLoudnormValuesForStats = null) => {
    ...
    args.jobLog(`GPU normalize split stats prepass enabled for ${planLabel} ${reason} rate=${statsSampleRate}`);
  ...
  const prefetchNextSplitStats = (plan, reason) => {
    ...
    for (let nextIdx = currentOrder + 1; nextIdx < processingPlans.length; nextIdx += 1) {
      ...
      if (splitStatsNeedsMeasuredValues(nextPlan) && !nextCpuValues) startCpuLoudnormTask(nextPlan, { background: true });
  ...
  const useStreamParallelism = useGpuSourcePort && useStreamingSourcePort && processingPlans.length > 1 && streamParallelism > 1;
  ...
  for (const plan of processingPlans) {
    ...
    if (useGpuSourcePort) {
      if (splitStatsForPlan) {
        startCpuLoudnormTask(plan, { background: true });
    ...
    if (useStreamingSourcePort) {
      ...
      const gpuPlan = [
        ...
        "--decode-command-json", q(JSON.stringify(decodeCommand)),
        "--encode-command-json", q(JSON.stringify(encodeCommand)),
        ...(splitStatsForPlan ? ["--stats-cache-input", q(splitStatsCacheInput || plan.statsCache)] : []),
        ...(usesStereoFallbackSourcePath(plan) ? ["--stereo-fallback-source-exact"] : []),
      ].join(" ");
      const gpuRes = await runShell(wrapRuntimeProfile(gpuPlan, plan, "streaming"), {
        args,
      ...
      logProfileStage(args, { scope: "plugin", name: "gpu_source_port_streaming", stream: plan.idx, source_stream: plan.sourceIdx, channels: plan.channels, wall_sec: gpuRes.wallSec });
      if (splitStatsForPlan) logProfileStage(args, { scope: "plugin", name: "gpu_source_port_streaming_total", stream: plan.idx, source_stream: plan.sourceIdx, channels: plan.channels, wall_sec: (Date.now() - streamStartedAt) / 1000 });
      if (gpuRes.code === 42) {
