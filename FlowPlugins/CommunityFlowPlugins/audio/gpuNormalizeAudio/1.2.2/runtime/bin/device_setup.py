...
from cuda_driver import alloc_pinned_host_buffer, chk, load_cuda, load_module
...
def create_host_io_buffers(cuda, cfg):
    ...
    pinned_requested = os.environ.get('LOUDNORM_GPU_PINNED_SYNC_IO') in ('1', 'true', 'TRUE', 'yes', 'YES')
    if pinned_requested and getattr(cuda, '_has_async_apply_api', False):
        try:
