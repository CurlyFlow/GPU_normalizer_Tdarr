...
def derive_runtime_config(args):
    ...
    source_precompute_from_channel_sums = source_exact_precompute and env_flag('LOUDNORM_GPU_STEREO_SOURCE_PRECOMPUTE_FROM_CHANNEL_SUMS', True)
    ...
    exact_stats_async_pipeline = exact_stats_fast_launch and not source_precompute_device_input and env_flag('LOUDNORM_GPU_STATS_ASYNC_PIPELINE')
    ...
    if source_stage_needed:
        ...
        source_precompute_stage_extra_windows = 29 + (stats_chunk_windows if (source_precompute_device_input or source_stage_fixed_buffer) else 0)
