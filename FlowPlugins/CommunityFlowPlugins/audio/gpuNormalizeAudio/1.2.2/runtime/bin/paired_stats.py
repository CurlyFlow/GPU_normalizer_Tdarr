...
def _new_runner(label, ctx):
    ...
    return _PairedStatsRunner(
        ...
        source_stage=None if cfg.source_precompute_device_energy else bytearray() if (cfg.source_precompute_in_stats or cfg.source_channel_hist4_exact or cfg.source_channel_hist4_boundary or cfg.source_channel_short_exact or cfg.source_channel_short_boundary) else None,
...
def _process_channel_stats_chunk(runner, chunk, chunks):
    ...
    if cfg.exact_stats_fast_launch:
        ...
        launch_kernel(ctx.cuda, f'{runner.label}-channel-stats-fast', channel_kernel_fn, channel_args, grid_x=args.channels)
        ...
        channel_dt = launch_timed_kernel(ctx.cuda, f'{runner.label}-channel-stats', channel_kernel_fn, channel_args, grid_x=args.channels)
    ...
    if cfg.source_precompute_device_energy:
        ...
        while full_ready_windows - runner.next_source_window >= cfg.source_precompute_windows:
            ...
            runner.source_stage_start_frame = _trim_source_energy_stage(
                ctx,
                runner.source_stage_start_frame,
...
def _process_combined_channel_stats_chunk(runner, chunk, chunks, *, combined_input_arg, input_channels, channel_offset, copy_dt=0.0):
    ...
    if runner.source_stage is not None:
        ...
        while full_ready_windows - runner.next_source_window >= cfg.source_precompute_windows:
            ...
            ctx.emit_progress('stats_sums', 0.70 * (chunks.done_bytes / float(cfg.nbytes)))
            runner.source_stage_start_frame = _trim_source_input_device_stage(
                ctx,
                runner.source_stage_start_frame,
