...
def emit_runtime_summary(args, cfg, *, gpu_name, elapsed, stats_result, gain_metrics, apply_result, bindings):
    ...
    print(f'audio_seconds={cfg.seconds:.3f} rate={args.rate} channels={args.channels} windows={cfg.windows} pcm_mib={cfg.mib:.1f} chunk_mib={cfg.chunk_bytes/(1024*1024):.1f} stats_chunk_mib={cfg.stats_chunk_bytes/(1024*1024):.1f} input_format={args.input_format} output_format={args.output_format} streaming_io={1 if cfg.streaming_io else 0} source_mode={source_mode} source_precompute_windows={cfg.source_precompute_windows} source_precompute_from_channel_sums={1 if cfg.source_precompute_from_channel_sums else 0} source_channel_hist4_exact={1 if cfg.source_channel_hist4_exact else 0} source_channel_hist4_boundary={1 if cfg.source_channel_hist4_boundary else 0} source_channel_short_exact={1 if cfg.source_channel_short_exact else 0} source_channel_short_boundary={1 if cfg.source_channel_short_boundary else 0} source_channel_short_margin_lu={cfg.source_channel_short_margin_lu:.6g} source_channel_short_halo_windows={cfg.source_channel_short_halo_windows} source_channel_hist4_margin_lu={cfg.source_channel_hist4_margin_lu:.6g} source_precompute_in_stats={1 if cfg.source_precompute_in_stats else 0} source_precompute_device_energy={1 if cfg.source_precompute_device_energy else 0} source_precompute_device_input={1 if getattr(cfg, "source_precompute_device_input", False) else 0} source_energy_packed={1 if cfg.source_energy_packed else 0} source_block_sums={1 if cfg.source_block_sums else 0} source_block_frames={cfg.source_block_frames} source_sum_audit={1 if cfg.source_sum_audit else 0}', file=sys.stderr)
    ...
    if cfg.source_exact_precompute:
        print(format_profile_stage('source_precompute', [
    ...
    print(format_profile_stage('apply', [
        ...
        ('exact_unsafe_skip_kernel_6ch', 1 if cfg.exact_unsafe_skip_kernel_6ch else 0),
        ...
        ('source_skip_limiter_lookahead_scan', 1 if cfg.source_skip_limiter_lookahead_scan else 0),
        ('source_fused_direct_feedback', 1 if cfg.source_fused_direct_feedback else 0),
        ('source_safe_run_scan_apply', 1 if cfg.source_safe_run_scan_apply else 0),
        ('exact_safe_run_scan_apply_6ch', 1 if cfg.exact_safe_run_scan_apply_6ch else 0),
